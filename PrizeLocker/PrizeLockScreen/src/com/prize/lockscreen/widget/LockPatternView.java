...
public class LockPatternView extends View {
	...
	private int mAspect;
	private int mRegularColor, mRegularPathColor;
	private int mErrorColor, mErrorPathColor;
	private int mSuccessColor, mSuccessPathColor;
	...
	public LockPatternView(Context context, AttributeSet attrs) {
		...
		mRegularPathColor = getResources().getColor(
				R.color.lock_pattern_regular_path_color);
		mErrorPathColor = getResources().getColor(
				R.color.lock_pattern_error_path_color);
		mSuccessPathColor = getResources().getColor(
				R.color.lock_pattern_success_path_color);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (drawPath) {
			mPathPaint.setColor(getCurrentPathColor(true /* partOfPattern */));
	 ...
	 * @return
	 */
	private int getCurrentPathColor(boolean partOfPattern) {
		if (!partOfPattern || mInStealthMode || mPatternInProgress) {
			// unselected circle
			return mRegularPathColor;
		} else if (mPatternDisplayMode == DisplayMode.Wrong) {
			// the pattern is wrong
			return mErrorPathColor;
		} else if (mPatternDisplayMode == DisplayMode.Correct
				|| mPatternDisplayMode == DisplayMode.Animate) {
			return mSuccessPathColor;
		} else {
			throw new IllegalStateException("unknown display mode "
					+ mPatternDisplayMode);
		}
