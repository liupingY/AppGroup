 ...
 */
public class GiftDownloadProgressButton extends TextView {
    ...
    public GiftDownloadProgressButton(Context context) {
        this(context, null);

    }

    public GiftDownloadProgressButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (!isInEditMode()) {
            mContext = context;
            initController();
...
//                canvas.drawRoundRect(bgRectf, mButtonRadius, mButtonRadius, mStokePaint);
                break;
        ...
        switch (mState) {
            ...
            case AppManagerCenter.APP_STATE_DOWNLOADED:
                switch (mState){
                    case AppManagerCenter.APP_STATE_UNEXIST:
                        if (!mIsGotWelfare) {
                            mCurrentText = mContext.getString(R.string.progress_btn_install_receive);
                        }else{
    ...
    public void setState(int state) {
        if (mState != state) {//状态确实有改变
            this.mState = state;
            invalidate();
            if (state == GiftDownloadProgressButton.INSTALLING) {
                //开启两个点动画
...
//    }
    ...
    public GiftDownloadProgressButton setCustomerController(ButtonController customerController) {
