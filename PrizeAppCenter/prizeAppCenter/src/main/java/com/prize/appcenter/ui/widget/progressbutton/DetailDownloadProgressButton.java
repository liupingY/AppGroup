 ...
 */
public class DetailDownloadProgressButton extends TextView {
...
//            setupAnimations();
        ...
        if (context instanceof AppCommentActivity) {
            isComment = true;
        }
...
//        mDot2Paint.setTextSize(mAboveTextSize);
    ...
    private int[] initGradientColor(int leftColor, int rightColor) {
        ...
        JLog.i(TAG, "initGradientColor--mBackgroundColor[0]=" + mBackgroundColor[0]);
...
//        mBackgroundColor[1] = rightColor;
        ...
        mBackgroundSecondColor = Color.parseColor("#f0f0f0");
...
//        mStokePaint.setColor(Color.RED);
//        mStokePaint.setColor(leftColor);
        isNewStyle=true;
        if (Build.VERSION.SDK_INT >= 21) {
            StateListAnimator animator = AnimatorInflater.loadStateListAnimator(mContext, R.drawable.btn_selector_animator);
            setStateListAnimator(animator);
            enabelDefaultPress(false);
        }
        this.invalidate();
    }
...
//                } else {
                ...
                mBackgroundPaint.setShader(mProgressBgGradient);
//                }
                JLog.i(TAG, "drawBackground--mBackgroundColor[0]=" + mBackgroundColor[0] +
                        "--mBackgroundSecondColor=" + mBackgroundSecondColor + "--buttonController.enableGradient()="
                        + buttonController.enableGradient());//1088776025
                canvas.drawRoundRect(mBackgroundBounds, mButtonRadius, mButtonRadius, mBackgroundPaint);
                if(!isNewStyle){
        ...
        switch (mState) {
            ...
            case AppManagerCenter.APP_STATE_DOWNLOADED:
                switch (mState) {
                    case AppManagerCenter.APP_STATE_UNEXIST:
                        if (isComment) {
                            mCurrentText = mContext.getString(R.string.uninstall_app);
                        } else {
                    ...
                    case AppManagerCenter.APP_STATE_INSTALLED:
                        if (isComment) {
                            mCurrentText = mContext.getString(R.string.app_comment);
                        } else {
...
//            this.mState = state;
//            invalidate();
//            if (state == DetailDownloadProgressButton.INSTALLING) {
//                //开启两个点动画
...
//    }
    ...
    public DetailDownloadProgressButton setCustomerController(ButtonController customerController) {
