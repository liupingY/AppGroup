...
public class ShareDialog extends AlertDialog implements OnClickListener {
	...
	private int[] moreStr;
	private String shareUrl = "http://www.szprize.cn/bingo.html";
	...
	public ShareDialog(Context context, int themeId) {
		...
		shareIcon = new int[] { R.drawable.umeng_socialize_qq,
				R.drawable.umeng_socialize_weixin,
		...
		shareStr = new int[] { R.string.QQ, R.string.weixin,
				R.string.QQ_circle, R.string.more, };
		...
		mShareButtonAdapter = new ShareButtonAdapter(this.context, list);
	...
	private void initSocialSDK() {
		...
		// 添加QQ平台
		UMQQSsoHandler qqHandler = new UMQQSsoHandler((Activity) context,
		...
		// 添加QQ空间平台
		QZoneSsoHandler qzoneHandler = new QZoneSsoHandler((Activity) context,
				"1104853719", "kC2RHki6G1WaxnJI");
	...
	private void setShareContent(BaseShareContent shareContent) {
		// 设置分享文字
		shareContent.setShareContent(context.getString(R.string.share_content)
				+ shareUrl);
		// 设置title
		shareContent.setTitle(context.getString(R.string.app_name));
		// 设置分享内容跳转URL
						...
						else 
						{
							...
							if (eCode == 40002) {
								Toast.makeText(context,
										context.getString(R.string.pls_install_qq),
