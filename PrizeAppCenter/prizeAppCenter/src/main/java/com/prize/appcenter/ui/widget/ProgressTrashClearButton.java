 ...
 * @author prize
 */
public class ProgressTrashClearButton extends ProgressBar {
    /**
     * 进度条画笔
     */
...
//    private Handler mHandler = new Handler();
    ...
    public ProgressTrashClearButton(Context context) {
        super(context);
        initPaint(context);
//        this.mcontext = context;
    }

    public ProgressTrashClearButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPaint(context);
//        this.mcontext = context;
    }

    public ProgressTrashClearButton(Context context, AttributeSet attrs, int defStyle) {
...
//        this.mcontext = context;
    ...
    protected void initPaint(Context context) {
        ...
        downloadingNomalBG = resource
                .getDrawable(R.drawable.progress_trash_clear_nomal);
        mDownLoadProgressDrawable = resource
                .getDrawable(R.drawable.progress_trash_clear_downloading);
        // 启动
        startNormalBG = resource
                .getDrawable(R.drawable.progress_trash_clear_nomal);
        startPressBG = resource
                .getDrawable(R.drawable.progress_trash_clear_press);
        mMask = resource.getDrawable(R.drawable.bg_update_dialog);

        installingBG = resource.getDrawable(R.drawable.progress_trash_clear_installing);
...
//    public void setGameInfo(AppsItemBean gameInfo, boolean isActivity) {
//        this.item = gameInfo;
//        if (isActivity){
//            postInvalidate();
//        }
//    }
        ...
        switch (state) {
            case AppManagerCenter.APP_STATE_WAIT:
                ...
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_wait, Color.parseColor("#54c590"));
                break;
            case AppManagerCenter.APP_STATE_DOWNLOADED:
                // 下载完成
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_install, Color.parseColor("#54c590"));
                break;
            case AppManagerCenter.APP_STATE_INSTALLED:
                // 安装完成(启动应用)
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_start, Color.parseColor("#54c590"));
                break;
            ...
            case AppManagerCenter.APP_STATE_UNEXIST:
                ...
                int color = Color.parseColor("#54c590");
                // 下载（没有下载的）
                    if (BaseApplication.isThird) {
                        textId = R.string.progress_btn_download;
                    } else {
                        textId = R.string.progress_trash_clear_btn_install;
                    }
    ...
    protected void drawDownloadBg(Canvas canvas, int textId) {
        ...
        drawText(canvas, text, x, y, Color.parseColor("#54c590"));
        canvas.save();
    }
    ...
    public void onClick() {
        ...
        int state = AIDLUtils.getGameAppState(item.packageName, item.id + "",
                item.versionCode);
        JLog.i("ProgressButton","onClick-state="+state);
        switch (state) {
            case AppManagerCenter.APP_STATE_DOWNLOADED:
                if(BaseApplication.isThird){
