...
public class SearchFlowLayout extends ViewGroup {
	...
	private static final float DEFAULT_MAX_HEIGHT = 150f;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (modeHeight == MeasureSpec.EXACTLY) {
			height = height <= mMaxHeight ? height
					: (int) mMaxHeight;
		}
		...
		// int maxHeightMeasureSpec = MeasureSpec.makeMeasureSpec(sizeHeight,
		// modeHeight);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < cCount; i++) {
			...
			if (childWidth + lp.leftMargin + lp.rightMargin + lineWidth > width) {
				...
				// if(mAllViews!=null&&mAllViews.size()>2){
				// break;
				// }
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		...
		mMaxHeight = a.getDimension(R.styleable.MaxHeightView_mhv_HeightDimen,
				DEFAULT_MAX_HEIGHT);
		a.recycle();
	}
