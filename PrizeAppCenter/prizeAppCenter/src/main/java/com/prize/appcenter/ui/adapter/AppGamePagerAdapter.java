...
import static com.prize.app.constants.Constants.TYPE_APP_LIST;
import static com.prize.app.constants.Constants.TYPE_FOCUS_WEB;
 ...
 * @author prize
 */
public class AppGamePagerAdapter extends GameListBaseAdapter {
    ...
    public AppGamePagerAdapter(RootActivity activity, boolean isGame) {
        super(activity);
    ...
    public int getItemViewType(int position) {
        ...
        if (itemtype.equals("focus")) {
            ...
            switch (type) {
                case Constants.WEB:
                    itemViewType = TYPE_FOCUS_WEB;
                    break;
                case Constants.TOPIC:
                    itemViewType = Constants.TYPE_FOCUS_TOPIC;
                    break;
                case Constants.RANK:
                    itemViewType = Constants.TYPE_FOCUS_RANK;
                    break;
                ...
                case Constants.HOTTEST:
                    itemViewType = Constants.TYPE_FOCUS_HOTTEST;
                    break;
        ...
        switch (type) {
            case Constants.TYPE_APP_LIST:
                ...
                if (bean.mAppItemBean != null) {
                    ...
                    if (isGame) {
                        bean.mAppItemBean = CommonUtils.formatAppPageInfo(bean.mAppItemBean, Constants.GAME_GUI, Constants.LIST, position + 1);
                    } else {
                        bean.mAppItemBean = CommonUtils.formatAppPageInfo(bean.mAppItemBean, Constants.APP_GUI, Constants.LIST, position + 1);
                    }
                    listViewItem.setData(bean.mAppItemBean, isGame, false, position + 1);//+1是为了从第一位开始算起
...
//                if (isGame) {
//                    bean.focus.app = CommonUtils.formatAppPageInfo(bean.focus.app, Constants.GAME_GUI, Constants.FOCUS, position + 1);
//                } else {
//                    bean.focus.app = CommonUtils.formatAppPageInfo(bean.focus.app, Constants.APP_GUI, Constants.FOCUS, position + 1);
//                }
            ...
            case Constants.TYPE_FOCUS_APPSIN:
                if (convertView == null) {
                    convertView = new CardAppsinView(mActivity, "", this.isGame);
                }
                ((CardAppsinView) convertView).setData(bean.focus);
                break;
            case Constants.TYPE_FOCUS_CATSIN:
                if (convertView == null) {
                    convertView = new CardCatsinView(mActivity, "", this.isGame);
                }
                ((CardCatsinView) convertView).setData(bean.focus);
                break;
            case Constants.TYPE_FOCUS_MATTS:
                if (convertView == null) {
                    convertView = new CardMattsGridView(mActivity, "", this.isGame);
                }
                ((CardMattsGridView) convertView).setData(bean.focus);
                break;
            case Constants.TYPE_FOCUS_RANK:
                if (convertView == null) {
                    convertView = new CardRankView(mActivity, this.isGame);
                    ((CardRankView) convertView).setmItemSelectCallBack(new ItemStateChangeCallBack() {
                        @Override
                        public void OnItemSelect(List<ExposureBean> beans, boolean isGame) {
                            if (mItemSelectCallBack != null) {
                                mItemSelectCallBack.OnItemSelect(beans, isGame);
                            }
                        }
                    });
                }
                ((CardRankView) convertView).setData(bean.focus, position + 1);
                break;
            case Constants.TYPE_FOCUS_TOPIC:
                if (convertView == null) {
                    convertView = new CardGridView(mActivity, "", this.isGame);
                }
                ((CardGridView) convertView).setData(bean, position + 1);
                break;
            case Constants.TYPE_FOCUS_HOTTEST:
                if (convertView == null) {//期刊类型
                    convertView = new FocusHottestView(mActivity, "", this.isGame);
                }
//                if (isGame) {
//                    bean.focus.app = CommonUtils.formatAppPageInfo(bean.focus.app, Constants.GAME_GUI, Constants.FOCUS, position + 1);
//                } else {
...
//                }
            ...
            case Constants.TYPE_FOCUS_VIDEO:
                if (convertView == null) {//视频类型
                    convertView = new FocusVideoView(mActivity, "", this.isGame);
                }
//                if (isGame) {
//                    bean.focus.app = CommonUtils.formatAppPageInfo(bean.focus.app, Constants.GAME_GUI, Constants.FOCUS, position + 1);
//                } else {
...
//                }
            ...
            case Constants.TYPE_FOCUS_NOTOPIC:
                if (convertView == null) {//无背景专题类型
                    convertView = new CardNoTopicView(mActivity, "", this.isGame);
                }
...
//        }
    ...
    private void updateView(String packageName) {
        ...
        for (int i = visiblePosition - headerViewsCount; i <= LastVisiblePosition - headerViewsCount; i++) {
            ...
            if (bean.type.equals("apps") && parentView instanceof ListViewItem) {
            ...
            } else if (bean.type.equals("focus")) { //专题下载刷新
                if (bean.focus.type.equals(Constants.TOPIC) && parentView instanceof CardGridView) {
                    ...
                    View viewBtn = subView.getDownLoadView();
                    if (viewBtn != null) {
                        viewBtn.invalidate();
                    }
                ...
                } else if (bean.focus.type.equals(Constants.VIDEO) && parentView instanceof FocusVideoView) { //视频下载刷新
    ...
    private void addDrawerView(ListViewItem subView, String appName, List<AppsItemBean> drawerData, int position) {
        ...
        DrawerGridView drawerGridView = new DrawerGridView(mActivity, title.toString(), isGame, false);
