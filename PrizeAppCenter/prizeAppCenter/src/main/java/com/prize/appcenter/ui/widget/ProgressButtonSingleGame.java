 ...
 * @author prize
 */
public class ProgressButtonSingleGame extends ProgressBar {
    /**
     * 进度条画笔
     */
...
//    private Handler mHandler = new Handler();
    ...
    public ProgressButtonSingleGame(Context context) {
        super(context);
        initPaint(context);
//        this.mcontext = context;
    }

    public ProgressButtonSingleGame(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPaint(context);
//        this.mcontext = context;
    }

    public ProgressButtonSingleGame(Context context, AttributeSet attrs, int defStyle) {
...
//        this.mcontext = context;
    ...
    protected void initPaint(Context context) {
        ...
        downloadingNomalBG = resource
                .getDrawable(R.drawable.progress_downing_singlegame_nomal);
//		downloadDrawable = resource.getDrawable(R.drawable.progress_bg_open);
        mDownLoadProgressDrawable = resource
                .getDrawable(R.drawable.progress_bg_singlegame_start_nomal);
//        // 启动
...
//        installGiftPressBG = resource.getDrawable(R.drawable.installgift_press);
        ...
        startPressBG = resource
                .getDrawable(R.drawable.progress_bg_singlegame_start_press);
        ...
        // openPressBG =
        // resource.getDrawable(R.drawable.progress_bg_open_press);
        installingBG = resource.getDrawable(R.drawable.bg_progress_singlegame_installing);
...
//    }
    ...
    private synchronized void drawButton(final Canvas canvas) {
        if (item == null) {
            return;
        }
    ...
    private  void drawRealButton(Canvas canvas, int state) {
        JLog.i("longbaoxiu", "drawRealButton-canvas=" + canvas + "--state=" + state);
        switch (state) {
            case AppManagerCenter.APP_STATE_WAIT:
                // 等待中 下载
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_wait, Color.parseColor("#FFFFFF"));
                break;
            case AppManagerCenter.APP_STATE_DOWNLOADED:
                // 下载完成
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_install,  Color.parseColor("#FFFFFF"));
                break;
            case AppManagerCenter.APP_STATE_INSTALLED:
                // 安装完成(启动应用)
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_start,  Color.parseColor("#FFFFFF"));
                break;
            ...
            case AppManagerCenter.APP_STATE_UPDATE:
                // 更新
                drawNormalBg(canvas, startNormalBG, startPressBG,
                        R.string.progress_btn_upload, Color.parseColor("#FFFFFF"));
                break;
    ...
    protected void drawDownloadBg(Canvas canvas) {
        ...
        drawText(canvas, text, x, y, Color.parseColor("#2b2b2b"));
        canvas.save();
    }
...
//		return true;
    ...
    protected void drawDownloadPause(Canvas canvas, int textId) {
        ...
        int x = getWidth() / 2;
        int y = (int) (getHeight() - (paintText.ascent() + paintText.descent())) / 2;
        drawText(canvas, text, x, y, Color.parseColor("#262626"));
        canvas.save();
    }
