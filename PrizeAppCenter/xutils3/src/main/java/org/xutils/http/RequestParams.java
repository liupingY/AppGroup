...
/**
 * Created by wyouflf on 15/7/17. 网络请求参数实体
 */
public class RequestParams {
	...
	private boolean cancelFast = false; // 是否可以被立即停止, true: 为请求创建新的线程,
										// 取消时请求线程被立即中断.
	 ...
	 */
	public RequestParams(String uri, ParamsBuilder builder, String[] signs,
			String[] cacheKeys) {
	...
	/* package */void init() throws Throwable {
		...
		if (httpRequest != null) {
			...
			buildCacheKey = builder
					.buildCacheKey(this, httpRequest.cacheKeys());
	 ...
	 *
	 * @param cancelFast
	 *            true: 为请求创建新的线程, 取消时请求线程被立即中断; false: 请求建立过程可能不被立即终止.
	 */
	...
	public void addBodyParameter(String name, Object value, String contentType) {
		...
		if (TextUtils.isEmpty(contentType)) {
			...
			this.fileParams.put(name, new BodyEntityWrapper<Object>(value,
					contentType));
		}
	}
			...
			if (!multipart && fileParams.size() == 1) {
				for (Object value : fileParams.values()) {
					...
					if (value instanceof File) {
						...
						result = new InputStreamBody((InputStream) value,
								contentType);
					} else if (value instanceof byte[]) {
						result = new InputStreamBody(new ByteArrayInputStream(
								(byte[]) value), contentType);
					}
					break;
				}
			} else {
	...
	private void checkBodyParams() {
		if (bodyParams != null
				&& (!TextUtils.isEmpty(bodyContent) || requestBody != null)) {
			if (this.queryStringParams == null) {
				this.queryStringParams = new HashMap<String, String>();
			}
			for (Map.Entry<String, String> entry : bodyParams.entrySet()) {
		...
		if (bodyParams != null
				&& (multipart || (fileParams != null && fileParams.size() > 0))) {
			if (this.fileParams == null) {
				this.fileParams = new HashMap<String, Object>();
			}
