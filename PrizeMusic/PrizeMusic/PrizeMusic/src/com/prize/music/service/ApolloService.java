...
import static com.prize.music.Constants.DATA_SCHEME;
...
import static com.prize.music.Constants.VISUALIZATION_TYPE;
...
public class ApolloService extends Service implements
	...
	//prize-public-bug:play stopped when file has been deleted-pengcancan-20060303
	public static final String PLAY_STOPPED = "com.andrew.apolloMod.playstoped";
	...
	//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-start
	private Handler mFileCheckHandler;
	//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-end
		...
		public void handleMessage(Message msg) {
			...
			case FOCUSCHANGE:
				...
				case AudioManager.AUDIOFOCUS_LOSS: 
					LogUtils.i(LOGTAG, "AudioFocus: received AUDIOFOCUS_LOSS");
					if (isPlaying()) {
				...
				case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
					LogUtils.i(LOGTAG, "AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK");
				...
				case AudioManager.AUDIOFOCUS_GAIN:
					LogUtils.i(LOGTAG, "AudioFocus: received AUDIOFOCUS_GAIN");
					...
					break;
				default:
					LogUtils.e(LOGTAG, "Unknown audio focus change code");
				}
				break;
			// case 999:
			// removeMessages(999);
			// mAppWidgetProvider4x2.notifyChange(ApolloService.this,
			// (String) msg.obj);
			// break;
			default:
				break;
	...
	public static final String UPDATELIST_ACTION = "android.intent.action.ACTION_UPDATELIST";
	private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(TelephonyManager.ACTION_PHONE_STATE_CHANGED)) {
				...
				Log.v("prize_zwl", "zwl-------->stop()");
				stop();
			}else if (UPDATELIST_ACTION.equals(action)) {
                setNextTrack();
                Log.i("pengcc", "I received a broadcast..");
            }
	...
	public void onCreate() {
		...
		Log.v("prize_zwl", "zwl-------->onCreate()");
		commandFilter.addAction(SHUTDOWN_ACTION);
		commandFilter.addAction(UPDATELIST_ACTION);
		...
		//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-start
		//mFileCheckHandler = new Handler();
		//mFileCheckHandler.postDelayed(mFileCheckRunnable, 5000);
		//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-end
		//prize-pubilc-bug:12606 cancel notification when booting-pengcancan-20160303-start
		cancelNotification();
		//prize-pubilc-bug:12606 cancel notification when booting-pengcancan-20160303-end
	}
	...
	public void onDestroy() {
		...
		//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-start
		//mFileCheckHandler.removeCallbacks(mFileCheckRunnable);
		//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-end
		cancelNotification();
		super.onDestroy();
	}
		...
		if (qlen > 1) {
			...
			try {
				crsr = MusicUtils.query(this,
						Audio.Media.EXTERNAL_CONTENT_URI, new String[] { "_id" },
	...
	public int onStartCommand(Intent intent, int flags, int startId) {		
		...
		setNextTrack();//prize-public-bug:15546 repeat mode not working when service been killed once-20160510-pengcancan
		//prize-pubilc-bug:12606 cancel notification when booting-pengcancan-20160303-end
...
//		mWakeLock.acquire();
		...
		if (intent != null && !isEventFromMonkey()) {
			String action = intent.getAction();
			String cmd = intent.getStringExtra("command");
			Log.d(TAG + "pcc","ApolloService onStartCommand() intent.action = " + action + ", cmd = " + cmd);
...
//					stopForeground(true);
	...
	private void openCurrentAndNext() {
		Log.i("pengcc", "openCurrentAndNext------->");
		synchronized (this) {
			if (mCursor != null) {
				mCursor.close();
				mCursor = null;
			...
			if (mCursor.getCount() <= 0) {
				LogUtils.i(LOGTAG, "mPlayPos=" + mPlayPos);
				/**
				 * fix bug
				 */
				if (mPlayPos > 0) {
			...
			//prize-public-bug:mCursor NullPointerException--pengcancan-20160324
			while (mCursor != null && !open(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/"
	...
	private void setNextTrack() {
		Log.i("pengcancan", "setNextTrack()--->"+mPlayer);
	...
	private boolean isWmaMusic(Cursor c){
		if (c != null && c.getCount() > 0) {
			String songpath = c.getString(c
					.getColumnIndex(AudioColumns.DATA));
...
*/			
			...
			if (mCursor == null) {
				...
				try {
					mCursor = resolver.query(uri, mCursorCols, where,
							selectionArgs, null);
					if (mCursor != null) {
						if (mCursor.getCount() == 0 && Integer.parseInt(Build.VERSION.SDK)<10 ) {
							mCursor.close();
							mCursor = null;
						} else {
							mCursor.moveToNext();
	...
	private void updateHistory() {
		...
		values.put(HistoryColumns.AUDIO_ARTIST, getArtistName());
		values.put(HistoryColumns.AUDIO_DURATION, duration());
		values.put(HistoryColumns.AUDIO_ID, getAudioId());
		values.put(HistoryColumns.AUDIO_TITLE, getTrackName());
...
//		startForeground(PLAYBACKSERVICE_STATUS, status);
	...
	/**
	 * @see when mRepeatMode == shuffle,current music history[].size ==0,user click prev Button.
	 *     this method will random a music to play
	 * @author lixing
	 * @return 
	 */
	private int getZeroPrevShufflePosition(){
		...
		int skip = mRand.nextInt(mPlayListLen);
		int cnt = -1;
		while (true) {
			while (tracks[++cnt] < 0)
				;
		...
		synchronized (this) {
			...
			do{
				...
				if (mShuffleMode == SHUFFLE_NORMAL) {
					...
					//prize-public-bug:mHistory NullPointerException--pengcancan-20160324
					if (mHistory != null) {
						int histsize = mHistory.size();
						if (histsize == 0) {
							// prev is a no-op
							int cnt = getZeroPrevShufflePosition();
							mHistory.add(0, cnt);
				...
				Log.d(TAG,"prev() opne failed openPrevFailed = " + openPrevFailed);
			}while(isWmaMusic(getCursorForId(mPlayList[mPlayPos])) && openPrevFailed < mPlayListLen);
	...
	public long getAudioId() {
		Log.i("pengcancan", "getAudioId()--->"+mPlayer);
		synchronized (this) {
	...
	public long duration() {
		Log.i("pengcancan", "duration()--->"+mPlayer);
		if (mPlayer != null && mPlayer.isInitialized()) {
	...
	public long position() {
		Log.i("pengcancan", "position()--->"+mPlayer);
		if (mPlayer != null && mPlayer.isInitialized()) {
	...
	public void setAudioSessionId(int sessionId) {
		Log.i("pengcancan", "setAudioSessionId()--->"+mPlayer);
		synchronized (this) {
			if (mPlayer != null && mPlayer.isInitialized()) {
	...
	public int getAudioSessionId() {
		Log.i("pengcancan", "getAudioSessionId()--->"+mPlayer);
		synchronized (this) {
			if (mPlayer != null && mPlayer.isInitialized()) {
	...
	public boolean isFavorite(long id) {
		...
		return MusicUtils.isFavorite(getApplicationContext(), id);
	}
	...
	public void removeFromFavorites(long id) {
		...
		MusicUtils.removeFromFavorites(getApplicationContext(), id);
	...
	public void addToFavorites(long id) {
		...
		MusicUtils.addToFavorites(getApplicationContext(), id);
		notifyChange(FAVORITE_CHANGED);
	}
		...
		MediaPlayer.OnErrorListener errorListener = new MediaPlayer.OnErrorListener() {
			...
			public boolean onError(MediaPlayer mp, int what, int extra) {
				...
				default:
					...
					LogUtils.e("MultiPlayer", "Error: " + what + "," + extra);
					break;
				}
	...
	static class ServiceStub extends IApolloService.Stub {
		...
		public String getName(StackTraceElement[] sts) {
			for (StackTraceElement st : sts) {
				LogUtils.i(LOGTAG, st.getFileName() + "--" + st.getLineNumber());
		...
		public void prev() {
			try {
				...
				// // }
				// }
				// LogUtils.i(LOGTAG, "prevsb=" + sb.toString());
				// }
			}
	...
	//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-start
	private Runnable mFileCheckRunnable = new Runnable() {
		
		@Override
		public void run() {
			Log.i(TAG+"pcc", "-mFileCheckRunnable--getPath(); "+getPath());
			if (getPath()!=null) {
				if (!isFileExisted(getApplicationContext(),getPath())) {
					Log.i(TAG+"pcc", "testtttttt");
					stop();
					sendBroadcast(new Intent(PLAY_STOPPED));
					cancelNotification();
					cancelNotification();
				}
			}
			mFileCheckHandler.postDelayed(this, 3000);
		}
	};
    ...
    public static String getFilePath(final Context mContext, final String filePath) {
        ...
        Log.i("pengcanccancan ","---->filePath:"+ filePath);
    ...
    public static boolean isFileExisted(Context ctx, String fileURIPath) {
        ...
        if (fileURIPath != null) {
            ...
            if (fileURIPath.startsWith("content://")) {
				if (fileURIPath.contains("internal")) {
					return true;
				}
            ...
            Log.i("pengcanccancan ",
            		"isFileExisted: " + result + " ,FileURIPath: " + fileURIPath + " ,Path: " + path);
        }
        return result;
    }
	//prize-public-bug:Music is still on while the file has been deleted-pengcancan-20160126-end
    ...
    public void onTaskRemoved(Intent rootIntent) {
    	...
    	Log.i("pengcancan", "Task has been removed.. rootIntent: " + rootIntent);
