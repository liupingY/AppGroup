...
public class MusicFloatView extends LinearLayout implements MusicUtils.Defs{
	...
	private void initUI() {
		//prize-public-bug:14026 float music window can't move --20160331-pengcancan-start
		...
		//View view = inflate(mContext, R.layout.layout_music_float_view, null);
		View view = inflate(mContext, R.layout.activity_suspend, null);
		
		mMusicFloatViewRl = (RelativeLayout)view.findViewById(R.id.suspend_relativelayout);
		mMusicFloatViewRl.setOnClickListener(mOperationListener);
		//mTrackNameFl = (LinearLayout)mMusicFloatViewRl.findViewById(R.id.show_ll);
		//mOperationFl = (LinearLayout)mMusicFloatViewRl.findViewById(R.id.progress_ll);
		
		mCurrentTime = (TextView)mMusicFloatViewRl.findViewById(R.id.suspend_progress_id);
		mTotalTime = (TextView)mMusicFloatViewRl.findViewById(R.id.suspend_totaltime_id);
		mProgress = (SeekBar)mMusicFloatViewRl.findViewById(R.id.suspend_seekBar_id);
		...
		mCloseButton = (ImageButton)mMusicFloatViewRl.findViewById(R.id.suspend_close_id);
		mCloseButton.setOnClickListener(mOperationListener);

		View v = (View) mTrackName.getParent();

		mPrevButton = (ImageButton)mMusicFloatViewRl.findViewById(R.id.suspend_previous_id);
		mPrevButton.setOnClickListener(mPrevListener);
		//mPrevButton.setRepeatListener(mRewListener, 260);
		mPauseButton = (ImageButton)mMusicFloatViewRl.findViewById(R.id.suspend_play_id);
		mPauseButton.requestFocus();
		mPauseButton.setOnClickListener(mPauseListener);
		mNextButton = (ImageButton)mMusicFloatViewRl.findViewById(R.id.suspend_next_id);
		mNextButton.setOnClickListener(mNextListener);
		//mNextButton.setRepeatListener(mFfwdListener, 260);
		mJumpToListButton = (ImageButton)mMusicFloatViewRl.findViewById(R.id.suspend_enter_id);
	...
	private void setPauseButtonImage() {
		try {
			if (mService != null && mService.isPlaying()) {
				mPauseButton.setBackground(mContext.getResources().getDrawable(R.drawable.suspend_pause_selector));
				if (!mSeeking) {
					mPosOverride = -1;
				}
			} else {
				mPauseButton.setBackground(mContext.getResources().getDrawable(R.drawable.suspend_play_selector));
			}
		} catch (RemoteException ex) {
		}
	}
	...
	private long refreshNow() {
		...
		try {
			...
			if ((pos > 0) && (mDuration > 0) && !"0:00".equals(time)) {
				...
				mCurrentTime.setText(time);
				String totalTime = MusicUtils.makeTimeString(mContext, mDuration / 1000);
		...
		try {
			if (mNeedUpdateDuration && mService.isPlaying()) {
				...
				if (newDuration > 0L && newDuration != mDuration) {
					...
					mTotalTime.setText(MusicUtils.makeTimeString(mContext, mDuration / soundToMs));
