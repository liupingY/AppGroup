...
public class LyricLoadHelper {
	private static final String TAG = "LyricLoadHelper";
	 ...
	 */
	public boolean loadLyric(String lyricPath, Activity a) {
		mHasLyric = false;
		...
		if (mHasLyric) {
			JLog.i(TAG,
					"Lyric file existed.Lyric has " + mLyricSentences.size()
							+ " Sentences");
		} else {
			JLog.i(TAG, "Lyric file does not existed");
		}
		return mHasLyric;
	}
	...
	private int seekSentenceIndex(long millisecond) {
		...
		try {
			...
			e.printStackTrace();
			JLog.i(TAG, "新的歌词载入了，所以产生了越界错误，不用理会，返回0");
			return 0;
		}
	}
...
//		}
	...
	public boolean loadLyric(String lyricPath) {
		JLog.i(TAG, "LoadLyric begin,path is:" + lyricPath);
		mHasLyric = false;
		mLyricSentences.clear();
	 ...
	 * @return String 
	 */
	public static String getFileName(String songName,String singer) {
		...
		pattern += ".lrc";
		return String.format(pattern,songName,
				singer);
	}
	/**
	 * 获取下载歌词文件路径
	 ...
	 * @return String 
	 */
	public static String getLrcFilePath(String songName,String singer) {
		return  Constants.LYRIC_SAVE_PATH+ File.separator+getFileName(songName, singer);
	}
	/**
	 * 
	 * 判断歌词文件是否存在
	 ...
	 * @return boolean 
	 */
	public static boolean isLrcFileExists(String songName,String singer) {
		String downloadFile = getLrcFilePath(songName,singer);
