...
public class SearchGameNetSource extends
	...
	protected GameData parseResp(SearchGameResp resp) {
		// gameSearchFromTag = NumberUtils.getByteValue(resp
		// .getGameSearchFromTag());
		// gameSearchFromInfo = resp.getGameSearchFromInfo();
		// boolean isFromThirdApp = (gameSearchFromTag == 2) ? true : false;
		// GameData data = new GameData();
		// ArrayList<Game> games = resp.getGames();
		...
		// if (games != null) {
		// for (Game game : games) {
		// item = new GameBean(game, Constants.SEARCH_GAMES_LISTCODE);
		// item.fromOtherSource = isFromThirdApp;
		// data.items.add(item);
		// }
		// }
		return data;
