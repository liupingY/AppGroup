...
import static com.prize.app.constants.Constants.PLAYLIST_FAVORITES;
...
public class PlaylistListFragment extends DragSortListViewFragment {
	...
	public PlaylistListFragment(Bundle args) {
		...
		mPlaylistName = getArguments().getString("playlistName");
	}
	...
	public boolean removePlaylistItem(int which, String audioIdsstring) {
		...
		if (mPlaylistId >= 0) {
			...
			if (mainActivity != null) {
				mainActivity.getContentResolver().delete(uri,
						Playlists.Members.AUDIO_ID + " in " + audioIdsstring,
						null);
			}
			return true;
...
//		popu_cancle.setOnClickListener(this);
		...
		li.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				table_name=(String) mLists.get(position).get("name");
	...
	public void setBackgroundAlpha(float bgAlpha) {
	    WindowManager.LayoutParams lp = ((Activity) mainActivity).getWindow()
	            .getAttributes();
	    lp.alpha = bgAlpha;
	    ((Activity) mainActivity).getWindow().setAttributes(lp);
	}
	// huanglingjun
...
//		// 设置显示在某控件下
//		myPopupWindow.showAsDropDown(play_add, 0, -50);
//		// 初始化菜单上的按键，并设置监听
...
//		popupContent.findViewById(R.id.popu_rename).setOnClickListener(this);
		...
		popupMenu=new PopupMenu(mainActivity, play_add);
		MenuInflater menuInflater = mainActivity.getMenuInflater();
        menuInflater.inflate(R.menu.menu_popmenu, popupMenu.getMenu());
        popupMenu.show();
...
//				}
	...
	public void processAction(String action) {
		...
		if (action.equals(Constants.ACTION_ADD)) {
			...
			JLog.i(TAG, "mLists.size()=" + mLists.size());
			if (mLists.size() > 0) {
				initAddPopu(mLists);
			} else {
	...
	class AddAsyncTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			if (action.equals(Constants.ACTION_ADD)
				...
				// modify by liukun for local music 2017.11.17  start 				
				List<MusicInfo> array = new ArrayList<MusicInfo>();
				for(int i=0;i<ids.length;i++){
					MusicInfo music_info = MusicUtils.audioIdToMusicInfo(getActivity(), ids[i]);
					array.add(music_info);
				}
				...
				// modify by liukun for local music 2017.11.17  end 
				if (dialog.isShowing()) {
...
//		
			...
			if (action.equals(Constants.ACTION_DELETE)) {
				...
				// modify by pengy audioplayer love show 
				List<MusicInfo> list = MusicUtils.audioIdToMusicInfo(mainActivity, audioIds);
				...
				// modify by pengy audioplayer love show 
				if(null!=mPlaylistName&&(mPlaylistName.equals("我喜欢的")||mPlaylistName.equals("My Love")))
				MusicUtils.removeMultiFromMyCollect(mainActivity, list, removeCallback, DatabaseConstant.TABLENAME_LOVE);
				return isExisted;
			}
			return false;
		}
