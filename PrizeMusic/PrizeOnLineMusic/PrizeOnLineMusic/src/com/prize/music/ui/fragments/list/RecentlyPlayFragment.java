...
//		mCursor = HistoryDao.getInstance(getActivity()).queryAllCursor(DatabaseConstant.TABLENAME_HISTORY);
...
//		mCursor = HistoryDao.getInstance(getActivity()).queryAllCursor(DatabaseConstant.TABLENAME_HISTORY);
		Cursor mAllCursor = HistoryDao.getInstance(getActivity()).queryAllCursor(DatabaseConstant.TABLENAME_HISTORY);
		long[] audioIds = null;
		if (mAllCursor != null && mAllCursor.getCount() > 0) {
			audioIds = new long[mAllCursor.getCount()];
			int i = 0;
			while (mAllCursor.moveToNext()) {
				int index = mAllCursor.getColumnIndex(HistoryColumns.HISTORY_BASE_ID);
				if (index != -1) {
