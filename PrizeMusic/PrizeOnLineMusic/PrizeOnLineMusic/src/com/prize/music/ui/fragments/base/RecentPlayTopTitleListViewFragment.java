...
public abstract class RecentPlayTopTitleListViewFragment extends
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		root = inflater.inflate(R.layout.toptitle_listview, container,
				false);
		mListView = (ListView) root.findViewById(android.R.id.list);
		action_back = (TextView) root.findViewById(R.id.action_back);
		action_title = (TextView) root.findViewById(R.id.action_title);
...
//				getActivity().finish();
	...
	public void processAction(String action) {
		...
		if (Constants.ACTION_BELL == action) {
			...
			// modify for loacl music 2016.06.14 start 
			...
			// modify for loacl music 2016.06.14 end 
		...
		return new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					...
					JLog.i("hu", "table_name=="+table_name+"---"+getString(R.string.my_love));
					final long[] selectIds = mAdapter.getSelectedAudioIds();
					if (selectIds == null || selectIds.length <= 0) {
						return;
					}
		...
		protected Boolean doInBackground(Void... params) {
			...
			if (action.equals(Constants.ACTION_SORT)) {
				...
				isExisted = MusicUtils.addTrackToPlaylist(getActivity(),
						new_id, ids);
				//modify for loacl music 2016.06.14  
//				if (dialog.isShowing()) {
