...
public abstract class ListViewAdapter extends SimpleCursorAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		long mRealId = GameDAO.getInstance().getUriIdByOnlineSongId(mPlayingId);
		Log.i("pengcancan", "mRealId : " + mRealId + ", mPlayingId : " + mPlayingId + ", mCurrentId : " + mCurrentId);
		if ((mPlayingId != 0 && mCurrentId != 0) && (mPlayingId == mCurrentId || mCurrentId == mRealId)) {
			viewholder.mPeakTwo.setVisibility(View.VISIBLE);
			mPeakTwoAnimation = (AnimationDrawable) viewholder.mPeakTwo
					.getDrawable();
			viewholder.mViewHolderLineOne.setTextColor(mContext.getResources().getColor(R.color.gold_color));
			viewholder.mViewHolderLineTwo.setTextColor(mContext.getResources().getColor(R.color.gold_color));
			try {
			...
			viewholder.mViewHolderLineOne.setTextColor(mContext.getResources().getColor(R.color.gray_black));
			viewholder.mViewHolderLineTwo.setTextColor(mContext.getResources().getColor(R.color.text_color_gray));
		}
		return view;
	}
	...
	public long[] getSelectedAudioIds() {
		...
		for (int i = 0; i < len; i++) {
			...
			int index = mCursor.getColumnIndex(HistoryColumns.HISTORY_BASE_ID);
			if (index < 0) {
