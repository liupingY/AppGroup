package com.prize.music.ui.dialog;
...
import com.prize.music.helpers.utils.DownloadManagerPro;
import com.prize.music.helpers.utils.PreferencesUtils;
import com.prize.music.helpers.utils.UpdateDataUtils;
import com.prize.music.receiver.CompleteReceiver;
import com.prize.music.receiver.CompleteReceiver.DownFinish;
import com.prize.music.ui.widgets.XRTextView;
 ...
 * @version V1.0
 */
public class UpdateSelfDialog extends Dialog implements android.view.View.OnClickListener, DownFinish {
	...
	//是否调起安装界面
	private volatile Boolean isInstallApk=false;

	public void setBean(AppsItemBean bean) {
		this.bean = bean;
	}

	public UpdateSelfDialog(Context context, int theme, String title, String subtitle, String content) {
		super(context, theme);
		mContext = context;
		this.title = title;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext.getContentResolver().registerContentObserver(DownloadManagerPro.CONTENT_URI, true, downloadObserver);
	...
	public void show() {
		...
		if (mContext != null) {
			mContext.getContentResolver().registerContentObserver(DownloadManagerPro.CONTENT_URI, true,
					downloadObserver);
		...
		case R.id.add_neg:
			...
			if (ClientInfo.networkType != ClientInfo.NONET) {
				if (ClientInfo.networkType == ClientInfo.WIFI) {
					//以后再说，WIFI情况同样下载，不提示，不调起安装
					downloadId = PreferencesUtils.getLong(mContext, Constants.KEY_NAME_DOWNLOAD_ID);
					...
					if (!TextUtils.isEmpty(Constants.APKFILEPATH)) {
						...
						if (file.exists()) {
							if (doFileExists(file,isInstallApk)) {
								return;
							}
						}
					}
					AppManagerCenter.pauseAllDownload();
					CompleteReceiver.isNeedInStall = false;
...
//			this.dismiss();
			...
			UpdateDataUtils.downloadApk(downloadManager, bean, mContext);
//			ToastUtils.showLongToast(mContext, "安装包后台下载中...");
			break;
		default:
			this.dismiss();
			break;
	 ...
	 * @param file
	 */
	private Boolean doFileExists(File file,Boolean isInstallApk) {
		PackageInfo packageInfo = mContext.getPackageManager().getPackageArchiveInfo(Constants.APKFILEPATH,
		...
		if (packageInfo != null && bean != null) {
			if (bean.packageName.equals(packageInfo.packageName) && bean.versionCode == packageInfo.versionCode) {
				if (MD5Util.Md5Check(Constants.APKFILEPATH, bean.apkMd5)) {
					if(isInstallApk)
					PackageUtils.installNormal(mContext.getApplicationContext(), Constants.APKFILEPATH);
					return true;
				} else {
					UpdateDataUtils.deletePrizeMusicApk(mContext, file, downloadManager);
					// mContext.getContentResolver()
					// .delete(MediaStore.Files
					// .getContentUri("external"),
					// "_DATA=?",
					// new String[] { Constants.APKFILEPATH });
					// downloadManager
					// .remove(PreferencesUtils.getLong(
					// mContext,
	...
	private class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				JLog.i("bao", "1.4.2=downing---8:success--16:faile--status=" + status);
				if (isDownloading(status)) {
					...
					if (status == DownloadManager.STATUS_FAILED) {
						...
						PreferencesUtils.putLong(mContext, Constants.KEY_NAME_DOWNLOAD_ID, -1);
...
//								doFileExists(file,isInstallApk);
							} else {
								PreferencesUtils.putLong(mContext, Constants.KEY_NAME_DOWNLOAD_ID, -1);
							}
						}
