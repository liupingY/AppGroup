...
public abstract class SongsListViewFragment extends RefreshableFragment
	...
	private AsyncLoader_GuessInfo deleteTask;
	//
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (getArguments() != null
			...
			mMusicLibraryFragment = (LocalMusicLibraryFragment) getArguments()
					.getSerializable("obj");
...
//		android.util.Log.d("tzm", "songitemclick: "+"num="+(currentTime - lastClickTime));
//		if (currentTime - lastClickTime > MIN_CLICK_DELAY_TIME) {
//		lastClickTime = currentTime;	
		//prize-add-bug:29635-tangzeming-20170309-end
		if (!isSelectMode) {
			//prize-add-bug:39209-liukun-20170721-start
			try {
			...
			//prize-add-bug:39209-liukun-20170721-end
		} else {
	...
	public void processAction(String action) {
		if (Constants.ACTION_BELL.equals(action)) {
			...
			for(int i=0;i<selectIds.length;i++){
				MusicInfo music_info = MusicUtils.audioIdToMusicInfo(getActivity(), selectIds[i]);
				array.add(music_info);
			}
			// modify by pengy for local music 2016.06.08  
			if (BaseApplication.SWITCH_UNSUPPORT) {
			    new AsyncLoader_GuessInfo(Constants.ACTION_SORT, selectIds).execute();
			}
			
			MusicUtils.AsyncAddAllMusicInfoToLocalTable(DatabaseConstant.TABLENAME_LOVE,array, getActivity(), new AddCollectCallBack() {
			...
			df = com.prize.music.ui.fragments.base.PromptDialogFragment
					.newInstance(getString(R.string.sure_you_want_to_delete) + " " + selectIds.length + " " + (selectIds.length>1 ? getString(R.string.songs_ask) : getString(R.string.song_ask)),
			...
			if(BaseApplication.SWITCH_UNSUPPORT){
				...
				MusicUtils.addAllMusicToTableDialog(getActivity(), array, callback,selectIds);
			}
	...
	private View.OnClickListener mDeletePromptListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			deleteTask=null;
			deleteTask =new AsyncLoader_GuessInfo(Constants.ACTION_DELETE, null);
			deleteTask.execute();
		}
	...
	class AsyncLoader_GuessInfo extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			if (action.equals(Constants.ACTION_DELETE)) {
				...
				String[] paths = MusicUtils.getAudioPaths(mContext, selectIds);
				if (paths == null) {
					return false;
				}
				...
				// modify by pengy for go  to next music 2016.07.05 
				if ((Arrays.binarySearch(selectIds, currentId)) >= 0) {
					try {
						if (MusicUtils.mService.getCurrentMusicInfoList().size() == len) {//prize-bug:20041 crash when deleting all local songs -pengcancan-20160817
			...
			// modify by pengy for local music 2016.06.08  start 
...
//		
			...
			// modify by pengy for local music 2016.06.08  end 
			return false;
		}
		...
		protected void onPostExecute(Boolean result) {
			...
			if (action.equals(Constants.ACTION_DELETE)) {
				if (result) {
					...
					ToastUtils.showToast(R.string.deleteFail);
		...
		return new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					...
					if(!TextUtils.isEmpty(table_name) && table_name.endsWith(getString(R.string.my_love))){
						MusicUtils.AsyncAddAllMusicInfoToLocalTable(DatabaseConstant.TABLENAME_LOVE,array, getActivity(),
					...
					// modify by pengy for local music 2016.06.14  end 
					if (isExisted) {
						ToastUtils.showOnceToast(getActivity(), getString(R.string.Song_has_been));
					} else {
						ToastUtils.showOnceToast(getActivity(), getString(R.string.addSuccessful));
