...
//		if (mImageInfo.data!=null&&mImageInfo.data.length>=4&&!TextUtils.isEmpty(mImageInfo.data[1])
//				&& !TextUtils.isEmpty(mImageInfo.data[2])
//				&& !TextUtils.isEmpty(mImageInfo.data[3])) {
//			SongDetailInfo info = new SongDetailInfo();
//			info.song_name = mImageInfo.data[3];
//			info.singers = mImageInfo.data[1];
//			info.album_name = mImageInfo.data[2];
//			if (DownloadHelper.isFileExists(info)) {
//				mXiamiSDK = new XiamiSDK(BaseApplication.curContext,
//						SDKUtil.KEY, SDKUtil.SECRET);
//				String filePath = FileUtils.getDownMusicFilePath(DownloadHelper
//						.getFileName(info));
//			return	mXiamiSDK.readAPIC(filePath, new BitmapFactory.Options());
//			}
//		}
		return null;
	}
