...
public class UpdateDataUtils {
	...
	// private static ArrayList<AppsItemBean> updateApps = new
	// ArrayList<AppsItemBean>();
	...
	public ArrayList<String> getDownLoadApkFiles() {
		...
		if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			String path = Environment.getExternalStorageDirectory().getAbsolutePath() + "/prizeAppCenter/download";
	 ...
	 */
	public static void deletePrizeMusicApk(Context context, File file,
		...
		PreferencesUtils.putString(context, Constants.APP_MD5, "");
	...
	public boolean removeInstalledApk(Context context, String packageName) {
		...
		try {
			...
			for (int i = 0; i < paths.size(); i++) {
				PackageInfo packageInfo = pm.getPackageArchiveInfo(paths.get(i), PackageManager.GET_ACTIVITIES);
				if (packageInfo != null) {
					if (packageName.equals(packageInfo.packageName)) {
						context.getContentResolver().delete(MediaStore.Files.getContentUri("external"), "_DATA=?",
	...
	public static void downloadApk(DownloadManager downloadManager, AppsItemBean bean, Context context) {
		File folder = Environment.getExternalStoragePublicDirectory(Constants.DOWNLOAD_FOLDER_NAME);
		...
		long saveId = PreferencesUtils.getLong(context, Constants.KEY_NAME_DOWNLOAD_ID);
		...
		try {
			DownloadManager.Request request = new DownloadManager.Request(Uri.parse(bean.downloadUrl));
			request.setDestinationInExternalPublicDir(Constants.DOWNLOAD_FOLDER_NAME, Constants.DOWNLOAD_FILE_NAME);
			// 设置文件类型
			MimeTypeMap mimeTypeMap = MimeTypeMap.getSingleton();
			String mimeString = mimeTypeMap
					.getMimeTypeFromExtension(MimeTypeMap.getFileExtensionFromUrl(bean.downloadUrl));
			...
			PreferencesUtils.putLong(context, Constants.KEY_NAME_DOWNLOAD_ID, downloadId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
