...
import static com.prize.app.constants.Constants.ALBUM_ID_KEY;
import static com.prize.app.constants.Constants.ALBUM_KEY;
import static com.prize.app.constants.Constants.ARTIST_ID;
import static com.prize.app.constants.Constants.ARTIST_KEY;
import static com.prize.app.constants.Constants.GENRE_KEY;
import static com.prize.app.constants.Constants.MIME_TYPE;
import static com.prize.app.constants.Constants.NUMALBUMS;
import static com.prize.app.constants.Constants.PLAYLIST_NAME;
...
import static com.prize.app.constants.Constants.UP_STARTS_ALBUM_ACTIVITY;
...
import com.prize.music.activities.TracksBrowser;
...
public class ApolloUtils {
	 ...
	 */
	public static View setHeaderLayout(String Type, Cursor cursor,
			Activity activity) {
		...
		// Artist image
		final ImageView mHanderImage = (ImageView) header
		...
		mInfo.data = (Type == TYPE_ALBUM ? new String[] { albumId, artistName,
		...
		headerText.setBackgroundColor((activity).getResources().getColor(
	 ...
	 * @param mListView
	 */
	public static void setListPadding(Fragment fragment, ListView mListView,
		...
		if (fragment.getArguments() != null) {
			...
			if (Audio.Albums.CONTENT_TYPE.equals(mimetype)) {
				mListView.setPadding(
						AQUtility.dip2pixel(fragment.getActivity(), left), top,
						AQUtility.dip2pixel(fragment.getActivity(), right),
						bottom);
	...
	public static boolean downloadFile(String urlString, File outFile) {
		LogUtils.i("0000", "图片下载地址urlString=" + urlString);
	...
	public static void startTracksBrowser(String Type, long id, Cursor mCursor,
			Context context) {
		Bundle bundle = new Bundle();
		if (Type == TYPE_ARTIST) {
			String artistName = mCursor.getString(mCursor
					.getColumnIndexOrThrow(ArtistColumns.ARTIST));
			LogUtils.i("test", "artistName=" + artistName);
			...
			String genreKey = mCursor.getString(mCursor
	...
	public static void startTracksBrowserForArtist(String Type, long id,
			Cursor mCursor, Context context) {
		Bundle bundle = new Bundle();// Audio.Albums.EXTERNAL_CONTENT_URI;
		if (Type == TYPE_ARTIST) {
			// String artistId = mCursor.getString(mCursor
			// .getColumnIndexOrThrow(BaseColumns._ID));
			String artistName = mCursor.getString(mCursor
					.getColumnIndexOrThrow(ArtistColumns.ARTIST));
			LogUtils.i("test", "artistName=" + artistName);
	...
	// huanglingjun
	public static void startTracksBrowser2(String Type, long id,
			Cursor mCursor, Context context, Long new_id) {
		Bundle bundle = new Bundle();
		if (Type == TYPE_ARTIST) {
			String artistName = mCursor.getString(mCursor
					.getColumnIndexOrThrow(ArtistColumns.ARTIST));
