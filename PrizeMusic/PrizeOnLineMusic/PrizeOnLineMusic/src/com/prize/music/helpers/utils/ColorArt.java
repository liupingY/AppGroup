...
public class ColorArt {
	...
	private int findEdgeColor() {
		...
		while (sortedColorIterator.hasNext()) {
			...
			double edgeColorRatio = (double) nextProposedColor.getCount()
					/ proposedEdgeColor.getCount();
	...
	private int colorWithMinimumSaturation(int color, float minSaturation) {
		...
		if (hsv[1] < minSaturation) {
			return Color
					.HSVToColor(new float[] { hsv[0], minSaturation, hsv[2] });
		}
	...
	private boolean isDistinctColor(int colorA, int colorB) {
		...
		if (Math.abs(r - r1) > threshold || Math.abs(g - g1) > threshold
				|| Math.abs(b - b1) > threshold || Math.abs(a - a1) > threshold) {
	...
	private class CountedColor implements Comparable<CountedColor> {
		...
		public int compareTo(CountedColor another) {
			return getCount() < another.getCount() ? -1
					: (getCount() == another.getCount() ? 0 : 1);
		}
		...
		public boolean isBlackOrWhite() {
			...
			if ((r > .91 && g > .91 && b > .91)
					|| (r < .09 && g < .09 && b < .09)) // color is white or
														// black
				return true;
	...
	public Drawable getDrawable(int colorhex, Orientation orientation){
		GradientDrawable mDrawable = new GradientDrawable(orientation, formatStringToint(decToHex(colorhex)));
		return mDrawable;
	}
	public Drawable getDrawable(String colorint, Orientation orientation){
		GradientDrawable mDrawable = new GradientDrawable(orientation, formatStringToint(colorint));
		return mDrawable;
	}
	...
	private int[] formatStringToint(String color){
		int[] colorint = new int[]{0,0,0,0};
		colorint[0] = stringToint("#"+color);
		String b=color.substring(2,color.length());
		colorint[1] = stringToint("#90"+b);
		colorint[2] = stringToint("#40"+b);
		colorint[3] = stringToint("#00"+b);
		return colorint;
	}
	private int stringToint(String strcolor){
		int n = Color.parseColor(strcolor);
		return n;
	}
}
