...
import com.prize.music.views.VerticalSeekBar;
import com.prize.music.R;

/**
 * @Note This displays specific track or album listings
 */
public class SimpleEq extends FragmentActivity implements
	...
	protected void onCreate(Bundle icicle) {
		...
		mPreferences = getSharedPreferences(Constants.APOLLO_PREFERENCES,
	...
	public void initEqualizerValues() {
		...
		bBoostEnable.setChecked(mPreferences.getBoolean(
				"simple_eq_boost_enable", false));
		eQEnable.setChecked(mPreferences.getBoolean(
				"simple_eq_equalizer_enable", false));
		int[] freqs = MusicUtils.getEqualizerFrequencies();
		for (int i = 0; i <= 5; i++) {
			SeekBars[i].setProgress(mPreferences.getInt("simple_eq_seekbars"
		...
		if (seekBar == bBoost) {
			...
			for (int i = 0; i <= 5; i++) {
				if (SeekBars[i] == seekBar) {
					ed.putInt("simple_eq_seekbars" + String.valueOf(i),
							progress);
				}
			}
		}
