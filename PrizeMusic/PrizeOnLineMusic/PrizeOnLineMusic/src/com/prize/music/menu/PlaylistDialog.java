...
package com.prize.music.menu;

import static com.prize.app.constants.Constants.INTENT_CREATE_PLAYLIST;
import static com.prize.app.constants.Constants.INTENT_KEY_DEFAULT_NAME;
import static com.prize.app.constants.Constants.INTENT_KEY_RENAME;
import static com.prize.app.constants.Constants.INTENT_PLAYLIST_LIST;
import static com.prize.app.constants.Constants.INTENT_RENAME_PLAYLIST;
...
public class PlaylistDialog extends FragmentActivity implements TextWatcher,
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			if (name != null && name.length() > 0) {
				...
				if (id >= 0) {
					...
					long new_id = MusicUtils.createPlaylist(
							PlaylistDialog.this, name);
					/*
					 * if (new_id >= 0) {
					 * MusicUtils.addToPlaylist(PlaylistDialog.this, mList,
					 * new_id); }
					 */
				}
				finish();
	...
	public void onCreate(Bundle icicle) {
		...
		if (INTENT_RENAME_PLAYLIST.equals(action)) {
			...
			mDefaultName = icicle != null ? icicle
					.getString(INTENT_KEY_DEFAULT_NAME) : mOriginalName;
		} else if (INTENT_CREATE_PLAYLIST.equals(action)) {
			mDefaultName = icicle != null ? icicle
					.getString(INTENT_KEY_DEFAULT_NAME) : makePlaylistName();
		...
		if (action != null && mRenameId >= 0 && mOriginalName != null
				|| mDefaultName != null) {
			...
			mPlaylistDialog.setView(mPlaylist, (int) (8 * dm.density),
					(int) (8 * dm.density), (int) (8 * dm.density),
					(int) (4 * dm.density));
			if (INTENT_RENAME_PLAYLIST.equals(action)) { // 保存
				mPlaylistDialog.setButton(Dialog.BUTTON_POSITIVE,
						getString(R.string.save), mRenamePlaylistListener);
				mPlaylistDialog.setOnShowListener(this);
			} else if (INTENT_CREATE_PLAYLIST.equals(action)) {
				mPlaylistDialog.setButton(Dialog.BUTTON_POSITIVE,
						getString(R.string.save), mCreatePlaylistListener);
			}
			mPlaylistDialog.setButton(Dialog.BUTTON_NEGATIVE, // 取消
	...
	public void onSaveInstanceState(Bundle outcicle) {
		...
		if (INTENT_RENAME_PLAYLIST.equals(action)) {
			outcicle.putString(INTENT_KEY_DEFAULT_NAME, mPlaylist.getText()
					.toString());
	...
	private void setSaveButton() {
		...
		if (typedname.trim().length() == 0
				|| PLAYLIST_NAME_FAVORITES.equals(typedname)) {
