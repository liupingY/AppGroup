...
public class LyricDownloadManager {
	private static final String TAG = "LyricDownloadManager";
	...
	public String searchLyricFromWeb(String musicName, String singerName,
			String oldMusicName) {
		LogUtils.i(TAG, "下载前，歌曲名:" + musicName + ",歌手名:" + singerName);
		...
		try {
			mUrl = new URL(strUrl);
			LogUtils.i(TAG, "请求获取歌词信息的URL：" + mUrl);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		try {
			...
			e1.printStackTrace();
			LogUtils.i(TAG, "http连接连接IO异常");
			return null;
		} catch (Exception e) {
			e.printStackTrace();
			LogUtils.i(TAG, "XML解析错误");
			return null;
		}
	...
	private String fetchLyricContent(String musicName, String singerName,
		...
		LogUtils.i(TAG, "歌词的真实下载地址:" + lyricURL);
		...
		if (content != null) {
			...
			String savePath = folderPath + File.separator + oldMusicName
					+ ".lrc";
			LogUtils.i(TAG, "歌词保存路径:" + savePath + "---oldMusicName="
					+ oldMusicName);
	...
	private void saveLyric(String content, String filePath) {
		...
		try {
			...
			out.write(content);
			out.close();
			LogUtils.i(TAG, "歌词保存成功");
		} catch (java.io.IOException e) {
			e.printStackTrace();
			LogUtils.i(TAG, "很遗憾，将歌词写入外存时发生了IO错误");
		}
	}
}
