...
public class LyricLoadHelper {
	...
	public boolean loadLyric(String lyricPath, Activity a) {
		...
		if (mHasLyric) {
			...
			LogUtils.i(TAG, "Lyric file does not existed");
		}
		return mHasLyric;
	}
	...
	private int seekSentenceIndex(long millisecond) {
		...
		try {
			...
			e.printStackTrace();
			LogUtils.i(TAG, "新的歌词载入了，所以产生了越界错误，不用理会，返回0");
			return 0;
		}
	}
	...
	public boolean loadLyric(String lyricPath) {
		LogUtils.i(TAG, "LoadLyric begin,path is:" + lyricPath);
		mHasLyric = false;
		mLyricSentences.clear();
