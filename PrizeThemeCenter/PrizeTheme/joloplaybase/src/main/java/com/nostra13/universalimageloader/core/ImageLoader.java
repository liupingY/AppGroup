...
import com.prize.app.BaseDetailFragment;
...
public class ImageLoader {
	 ...
	 * Default {@linkplain DisplayLargerImageOptions display image options} from
	 ...
	 *            Image aware view} which should display image
	 * @param options
	 *            {@linkplain com.nostra13.universalimageloader.core.DisplayLargerImageOptions
	 *            Options} for image decoding and displaying. If <b>null</b> -
	 *            default display image options
	 *            {@linkplain ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayLargerImageOptions)
	 ...
	 */
	public void displayImage(String uri, ImageAware imageAware,
			DisplayLargerImageOptions options) {
	 ...
	 */
	public void displayImage(String uri, ImageAware imageAware,
			DisplayLargerImageOptions options, ImageLoadingListener listener) {
	 ...
	 */
	public void displayImage(String uri, ImageAware imageAware,
			DisplayLargerImageOptions options, ImageSize targetSize,
			ImageLoadingListener listener,
			ImageLoadingProgressListener progressListener) {
		checkConfiguration();
		if (imageAware == null) {
			...
			if (options.shouldPostProcess()) {
				...
				if (bmp.getHeight() < bmp.getWidth()) {
					if (detailFragment != null) {
						detailFragment.loadSuccess(bmp);
					}
				}
//				bmp = rotaingImageView(90, bmp);
			...
			mDisplayTask = new LoadAndDisplayImageTask(
					engine, imageLoadingInfo, defineHandler(options));
//			mDisplayTask.setRotate(isRotaing);
			mDisplayTask.setFragment(detailFragment);
			if (options.isSyncLoading()) {
				mDisplayTask.run();
			} else {
	...
	private LoadAndDisplayImageTask mDisplayTask;
	/**
	...
	public void displayImage(String uri, ImageView imageView,
		...
		displayImage(uri, new ImageViewAware(imageView), null, targetImageSize,
				null, null);
	}
	...
	public void displayImage(String uri, ImageView imageView,
			DisplayLargerImageOptions options, ImageLoadingListener listener,
			ImageLoadingProgressListener progressListener) {
		displayImage(uri, new ImageViewAware(imageView,false), options, listener,
				progressListener);
	}
	 ...
	 *             called before
	 */
	public void loadImage(String uri, DisplayLargerImageOptions options,
			ImageLoadingListener listener) {
	 ...
	 *             called before
	 */
	public Bitmap loadImageSync(String uri, DisplayLargerImageOptions options) {
	...
	public Bitmap loadImageSync(String uri, ImageSize targetImageSize,
		...
		options = new DisplayLargerImageOptions.Builder().cloneFrom(options)
	...
	public void setDefaultLoadingListener(ImageLoadingListener listener) {
		defaultListener = listener == null ? new SimpleImageLoadingListener()
				: listener;
	}
	...
	private static Handler defineHandler(DisplayLargerImageOptions options) {
	...
	public void setRotate(boolean rotaing) {
		isRotaing = rotaing;
	}
	...
	private Bitmap rotaingImageView(int angle, Bitmap bitmap) {
		...
		if (height >= wight) {
			...
			bitmap = Bitmap.createBitmap(bitmap, 0, 0, wight, height, matrix,
					true);
		}
		return bitmap;
	}
	
	public BaseDetailFragment detailFragment;
	public void setFragment(BaseDetailFragment detailFragment) {
