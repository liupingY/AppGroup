...
/**
 * Created by wyouflf on 15/7/23. 使用sqlite索引实现的LruDiskCache
 */
public final class LruDiskCache {

	private static final HashMap<String, LruDiskCache> DISK_CACHE_MAP = new HashMap<String, LruDiskCache>(
			5);
	...
	private LruDiskCache(String dirName) {
		...
		if (this.cacheDir != null
				&& (this.cacheDir.exists() || this.cacheDir.mkdirs())) {
			available = true;
		}
		deleteNoIndexFiles();
	}
	...
	public DiskCacheFile getDiskCacheFile(String key) {
		...
		if (entity != null && new File(entity.getPath()).exists()) {
			ProcessLock processLock = ProcessLock.tryLock(entity.getPath(),
					false, LOCK_WAIT);
			if (processLock != null && processLock.isValid()) {
				result = new DiskCacheFile(entity, entity.getPath(),
						processLock);
				if (!result.exists()) {
					try {
	...
	public DiskCacheFile createDiskCacheFile(DiskCacheEntity entity)
			throws IOException {
		if (!available || entity == null) {
			return null;
		}
		...
		entity.setPath(new File(this.cacheDir, MD5.md5(entity.getKey()))
				.getAbsolutePath());
	 ...
	 * @param cacheFile
	 */
	/* package */DiskCacheFile commitDiskCacheFile(DiskCacheFile cacheFile)
			throws IOException {
		...
		if (cacheFile.getName().endsWith(TEMP_FILE_SUFFIX)) { // is temp file
			...
			try {
				...
				if (processLock != null && processLock.isValid()) { // lock
					destFile = new DiskCacheFile(cacheEntity, destPath,
							processLock);
		...
		trimExecutor.execute(new Runnable() {
			...
			public void run() {
				if (available) {
					...
					// trim db
					try {
						int count = (int) cacheDb.selector(
								DiskCacheEntity.class).count();
						if (count > LIMIT_COUNT + 10) {
							List<DiskCacheEntity> rmList = cacheDb
									.selector(DiskCacheEntity.class)
									.orderBy("lastAccess").orderBy("hits")
									.limit(count - LIMIT_COUNT).offset(0)
									.findAll();
	...
	private void deleteExpiry() {
		try {
			WhereBuilder whereBuilder = WhereBuilder.b("expires", "<",
					System.currentTimeMillis());
			List<DiskCacheEntity> rmList = cacheDb
					.selector(DiskCacheEntity.class).where(whereBuilder)
					.findAll();
			if (rmList != null && rmList.size() > 0) {
				// delete cache files
				for (DiskCacheEntity entity : rmList) {
