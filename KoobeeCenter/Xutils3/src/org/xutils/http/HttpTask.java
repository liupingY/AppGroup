 ...
 */
public class HttpTask<ResultType> extends AbsTask<ResultType> implements
		ProgressHandler {
	...
	private static final PriorityExecutor HTTP_EXECUTOR = new PriorityExecutor(
			5);
	private static final PriorityExecutor CACHE_EXECUTOR = new PriorityExecutor(
			5);
	...
	private UriRequest initRequest() throws Throwable {
		...
		if (callback instanceof Callback.TypedCallback) {
			...
			loadType = ParameterizedTypeUtil.getParameterizedType(callBackType,
					Callback.PrepareCallback.class, 0);
		} else {
			loadType = ParameterizedTypeUtil.getParameterizedType(callBackType,
	...
	protected ResultType doBackground() throws Throwable {
		...
		if (cacheCallback != null
				&& HttpMethod.permitsCache(params.getMethod())) {
			...
			while (retry) {
				try {
					...
					exception = ex;
					retry = retryHandler.retryRequest(ex, ++retryCount,
							this.request);
				}
			}
		...
		while (retry) {
			...
			try {
				...
				try {
					clearRawResult();
					requestWorker = new RequestWorker(this.request,
							this.loadType);
				...
				if (prepareCallback != null) {
					try {
						result = (ResultType) prepareCallback
								.prepare(rawResult);
					} finally {
						clearRawResult();
					}
				...
				if (this.request.getResponseCode() == 304) { // disk cache is
																// valid.
					return null;
				} else {
					exception = ex;
		...
		case FLAG_CACHE: {
			synchronized (cacheLock) {
				try {
					...
					trustCache = this.cacheCallback
							.onCache((ResultType) args[0]);
				} catch (Throwable ex) {
					trustCache = false;
			...
			if (this.progressCallback != null && args.length == 3) {
				try {
					this.progressCallback.onLoading(
							...
							((Number) args[1]).longValue(), (Boolean) args[2]);
				} catch (Throwable ex) {
					callback.onError(ex, true);
				}
			}
		...
		if (progressCallback != null && request != null && total > 0) {
			...
			if (forceUpdateUI) {
				...
				if (currTime - lastUpdateTime >= RATE) {
					...
					this.update(FLAG_PROGRESS, total, current,
							request.isLoading());
				}
			}
		}
	...
	/**
	 * 请求发送和加载数据线程. 该线程被join到HttpTask的工作线程去执行. 它的主要作用是为了能强行中断请求的链接过程;
	 * 并辅助限制同时下载文件的线程数. but: 创建一个Thread约耗时2毫秒, 优化?
	 */
