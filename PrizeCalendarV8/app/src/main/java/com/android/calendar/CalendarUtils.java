...
public class CalendarUtils {
    ...
    public static class TimeZoneUtils {
        ...
        /*
         * M: The Runnables should be sorted by threads which the Runnables came
         * from. The Runnable came from the same thread should be save into the
         * same HashSet.
         */
        private static HashMap<Looper, HashSet<Runnable>> mTZCallbacks = new HashMap<Looper, HashSet<Runnable>>();
        ...
        /*
         * M: message ID used to handle time zone update
         */
        private static final int EVENT_DO_TIMEZONE_UPDATE = 1;
        ...
        private class AsyncTZHandler extends AsyncQueryHandler {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                synchronized (mTZCallbacks) {
                    ...
                    /**M: check if the home tz has been initialed before write prefs. @{*/
                    SharedPreferences prefs = getSharedPreferences((Context)cookie, mPrefsName);
                    String homeTz = prefs.getString(KEY_HOME_TZ, null);
                    LogUtil.oi(TAG, "homeTz:" + homeTz);
                    /**@}*/
                    while (cursor.moveToNext()) {
                        String key = cursor.getString(keyColumn);
                        String value = cursor.getString(valueColumn);
                        LogUtil.oi(TAG,"onQueryComplete, key:" + key + "; value:" + value);
                    ...
                    /*
                     * M: The runnables belonged to the current thread, just run
                     * it here, the others should be posted to the right thread
                     * to implement.
                     */
                    Looper current = getLooper();
                    Set<Looper> looperSet = new HashSet<Looper>(mTZCallbacks.keySet());
                    for (Looper looper : looperSet) {
                        HashSet<Runnable> runnableSet = mTZCallbacks.get(looper);
                        if (looper == current) {
                            for (Runnable callback : runnableSet) {
                                if (callback != null) {
                                    callback.run();
                                }
                            }
                            mTZCallbacks.remove(looper);
                        } else {
                            postTZCallbackToOriginalThread(looper);
                        }
                    }
                }
            }
        ...
        public String getTimeZone(Context context, Runnable callback) {
            synchronized (mTZCallbacks){
                if (mFirstTZRequest) {
                    ...
                    /*
                     * M: Maybe there is no looper in some work thread, it should
                     * ignore the calls came from those threads.
                     */
                ...
                if (mTZQueryInProgress) {
                    /*
                     * M: enqueue the time zone callback.
                     */
                    enqueueTimeZoneCallback(callback);
                }
            }
        ...
        /*
         * M: Put the runnable to the correct HashSet sorted by the
         * thread which the runnable belonged to.
         */
        private void enqueueTimeZoneCallback(Runnable callback) {
            Looper myLooper = Looper.myLooper();
            HashSet<Runnable> tempHashSet = mTZCallbacks.get(myLooper);
            if (tempHashSet == null) {
                tempHashSet = new HashSet<Runnable>();
                mTZCallbacks.put(myLooper, tempHashSet);
            }
            tempHashSet.add(callback);
        }

        /*
         * M: Post the time zone callback to run in its the original thread.
         */
        private void postTZCallbackToOriginalThread(Looper looper) {
            Handler tzHandler = new Handler(looper) {
                @Override
                public void handleMessage(Message msg) {
                    if (msg.what == TimeZoneUtils.EVENT_DO_TIMEZONE_UPDATE) {
                        synchronized (mTZCallbacks) {
                            if ((Looper) msg.obj != getLooper()) {
                                Log.i(TAG, "sent to error looper, msg=" + msg);
                            }
                            HashSet<Runnable> hashSet = mTZCallbacks.get((Looper) msg.obj);
                            for (Runnable callback : hashSet) {
                                if (callback != null) {
                                    callback.run();
                                }
                            }
                            mTZCallbacks.remove((Looper) msg.obj);
                            Log.i(TAG, "handle time zone update in original thread, thread=" + Thread.currentThread());
                        }
                    } else {
                        Log.i(TAG, "unkown message msg=" + msg);
                    }
                }
            };

            Message msg = tzHandler.obtainMessage(
                    TimeZoneUtils.EVENT_DO_TIMEZONE_UPDATE,
                    looper);
            tzHandler.sendMessage(msg);
        }
    }
