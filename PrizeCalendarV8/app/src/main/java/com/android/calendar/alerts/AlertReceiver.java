        ...
        if (MAP_ACTION.equals(intent.getAction())) {
            ...
            if (eventId != -1) {
                ...
                if (geoIntent != null) {
                    ...
                    /// M:Avoid calendar crashes when there is no map application installed @{
                    try {
                        context.startActivity(geoIntent);
                    } catch (ActivityNotFoundException e) {
                        Toast.makeText(context, R.string.noApplications, Toast.LENGTH_SHORT).show();
                        if (AlertService.DEBUG) {
                            Log.w(TAG, "onReceive: fail to start activity, no activity found to handle intent "
                                    + geoIntent.toString());
                        }
                    }
                    /// @}
...
//                mStartingService = pm.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP |PowerManager.SCREEN_DIM_WAKE_LOCK,"StartingAlertService");//提前亮屏
//                mStartingService = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK |PowerManager.ON_AFTER_RELEASE,"StartingAlertService"); // 闹钟的亮屏方法，日历不亮
            ...
            try {
                ...
                Log.w(TAG, "fail to start AlertService, ignore", ex);
            }
        }
    }
    ...
    private static PendingIntent createClickEventIntent(Context context, long eventId,
        ...
        return createDismissAlarmsIntent(context, eventId, startMillis, endMillis, notificationId,
                DismissAlarmsService.SHOW_ACTION, isOverdue);
    }
    ...
    private static PendingIntent createDeleteEventIntent(Context context, long eventId,
            long startMillis, long endMillis, int notificationId, boolean isOverdue) {
        return createDismissAlarmsIntent(context, eventId, startMillis, endMillis, notificationId,
                DismissAlarmsService.DISMISS_ACTION, isOverdue);
    }
    ...
    private static PendingIntent createDismissAlarmsIntent(Context context, long eventId,
        ...
        /// M: This action means we will opened the event after it been clicked. @{
        if (action.equals(DismissAlarmsService.SHOW_ACTION)) {
            intent.putExtra(AlertUtils.EVENT_SHOWED, true);
        }
        /// @}
    ...
    private static PendingIntent createAlertActivityIntent(Context context) {
        ...
        /*
         * M: Comment out the flag PendingIntent.FLAG_ONE_SHOT to make sure that
         * the low priority events notification could be clicked every time, no
         * need to depend the update operation finished.
         */
    ...
    private static URLSpan[] getURLSpans(Context context, long eventId) {
        ...
        /** M: use try...catch.. to close cursor. */
        try {
