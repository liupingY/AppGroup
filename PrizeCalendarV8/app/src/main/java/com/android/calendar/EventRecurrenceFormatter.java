...
public class EventRecurrenceFormatter
{
    ...
    /**M: record the system locale to decide whether it should re-cache month repeat strings.*/
    ...
    private static void cacheMonthRepeatStrings(Resources r, int weekday) {
        boolean needRefresh = !Locale.getDefault().toString().equals(mLocale);
        if(needRefresh) {
            mLocale = Locale.getDefault().toString();
        }
        if (mMonthRepeatByDayOfWeekIds == null || needRefresh) {
        ...
        if (mMonthRepeatByDayOfWeekStrs[weekday] == null || needRefresh) {
