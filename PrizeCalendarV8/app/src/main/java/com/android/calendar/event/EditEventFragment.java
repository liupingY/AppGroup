...
public class EditEventFragment extends Fragment implements CalendarController.EventHandler, OnColorSelectedListener {
    ...
    ///M: #DatePickerDialog# used to restore DatePickerDialog
    private static final String BUNDLE_KEY_SHOW_DATE_PICKER = "show_date_picker";
    private static final String BUNDLE_SELECTED_DATE = "selected_date";
    private boolean mShowDatePicker = false;
    private int[] mSelectedDate = null;
    ...
    private Button mPrizebuttoncancel;
    private Button mPrizebuttondone;
    /**
    ...
    public EditEventFragment(CalendarController.EventInfo event, ArrayList<ReminderEntry> reminders,
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mPrizebuttoncancel = (Button)getActivity().findViewById(R.id.cancel);
        mPrizebuttondone = (Button)getActivity().findViewById(R.id.confirm);
        mPrizebuttondone.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
            	//prize-Judage starttime isn't grenter then endtime , when gendter then enttime  please let uses new choose -lixing-2015-7-27--start
            	if(mView.mStartTime.toMillis(true) > mView.mEndTime.toMillis(true)){           		
            		Toast.makeText(getActivity(), getString(R.string.time_error), 200).show();
            		return;
            	}
            	//prize-Judage starttime isn't grenter then endtime , when gendter then enttime  please let uses new choose -lixing-2015-7-27--end
            	Log.d("EditEvent","startTime is:" + mView.mStartTime.toMillis(true) + ", endTime is:" + mView.mEndTime.toMillis(true));
//            	01-01 01:09:49.707: D/EditEvent(8102): startTime is:1420045760000, endTime is:1420041600000
//            	01-01 01:15:55.173: D/EditEvent(8831): startTime is:1420046137000, endTime is:1420046137000
//            	01-01 01:18:46.473: D/EditEvent(8831): startTime is:1420046309000, endTime is:1420128000000
//            	01-01 01:20:21.713: D/EditEvent(8831): startTime is:1420046347000, endTime is:1420128000000
        ...
        mPrizebuttoncancel.setOnClickListener(new View.OnClickListener() {
        	@Override
        	public void onClick(View v) {
        		 mOnDone.setDoneCode(Utils.DONE_REVERT);
                 mOnDone.run();
...
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
        ...
        mView = new EditEventView(this, view, mOnDone, mTimeSelectedWasStartTime,
                mDateSelectedWasStartDate, mShowDatePicker, mSelectedDate,false);
        startQuery();
        ...
        /*mView.mPrizebuttondone.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
            	prize-Judage starttime isn't grenter then endtime , when gendter then enttime  please let uses new choose -lixing-2015-7-27--start
            	if(mView.mStartTime.toMillis(true) > mView.mEndTime.toMillis(true)){           		
...
//            	01-01 01:20:21.713: D/EditEvent(8831): startTime is:1420046347000, endTime is:1420128000000
        ...
        mView.mPrizebuttoncancel.setOnClickListener(new View.OnClickListener() {
        	@Override
        	public void onClick(View v) {
        		 mOnDone.setDoneCode(Utils.DONE_REVERT);
                 mOnDone.run();
        ...
        /*prize-ʾ--lixing-2015-6-24-start*/
        mView.mPrizebuttonreturn.setOnClickListener(new View.OnClickListener() {			
			@Override
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            ///M: #DatePickerDialog# get state information of DatePickerDialog
            if (savedInstanceState.containsKey(BUNDLE_KEY_SHOW_DATE_PICKER)) {
                mShowDatePicker = savedInstanceState.getBoolean(BUNDLE_KEY_SHOW_DATE_PICKER);
            }
            if (savedInstanceState.containsKey(BUNDLE_SELECTED_DATE)) {
                mSelectedDate = savedInstanceState.getIntArray(BUNDLE_SELECTED_DATE);
            }
        }
    }
    ...
    class Done implements EditEventHelper.EditDoneRunnable {
        ...
        public void run() {
            ...
            /** M: check low storage state. @{ */
            if ((mCode & Utils.DONE_SAVE) != 0 && MTKUtils.isLowStorage(mActivity)) {
                MTKUtils.toastLowStorage(mActivity);
                Log.w(TAG, "Done runnable, in low storage state, not to save!");
            ...
            if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
                ...
                if (!mModel.mAttendeesList.isEmpty()
                        /// M: Local account can't invite guest, exclude it.
                        && !CalendarContract.ACCOUNT_TYPE_LOCAL.equals(mModel.mAccountType)) {
                    if (mModel.mUri != null) {
    ...
    /** M: Keep behavior of android 4.2, create no title event when either location
    ...
    public void onDestroy() {
        ///M: moved in from onPause() method @{
    ...
    public void onSaveInstanceState(Bundle outState) {
        mView.prepareForSave();
        ///M: Don't save mModel if query hasn't been completed, because UI issue will happen
        ///when using mRestoreModel . @{
        if (mOutstandingQueries == 0) {
            outState.putSerializable(BUNDLE_KEY_MODEL, mModel);
        } else {
            outState.putSerializable(BUNDLE_KEY_MODEL, null);
        }
        ///@}
        ...
        ///M: #DatePickerDialog# save information about DatePickerDialog
        outState.putBoolean(BUNDLE_KEY_SHOW_DATE_PICKER, mView.mShowDatePicker);
        outState.putIntArray(BUNDLE_SELECTED_DATE, mView.mSelectedDate);
...
///TODO: extract these code out
    ...
    private void formatOriginalRrule() {
        //TODO: MTK fix the issue or remove this function.
//        int position = mView.mRepeatsSpinner.getSelectedItemPosition();
//        /*
//         * M: Maybe the position is equal to -1 sometimes, it should skip this
//         * case.
//         */
//        if (position >= 0) {
//            int selection = mView.getRecurrenceIndexes().get(position);
//            EditEventHelper.updateRecurrenceRule(selection, mOriginalModel,
...
//        }
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == PrizeSelectActivity.REQUEST_SELECTED && resultCode == Activity.RESULT_OK) {
			int mSel = data.getIntExtra("selected", 0);
			mView.updateReminders(mSel);
		}
	}
