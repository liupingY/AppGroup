...
public class RecurrencePickerDialog extends DialogFragment implements OnItemSelectedListener,
    ...
    /**
     * M: Save the time duration to re-use.
     */
    ...
    /*
     * M: Calendar event end time used to limit the repeats rule.
     */
    public static final String BUNDLE_EVENT_TIME_DURATION = "budle_event_time_duration";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedInstanceState != null) {
            ...
            /**
             * M: restore the event time duration.
             */
            mTimeDuration = savedInstanceState.getLong(BUNDLE_EVENT_TIME_DURATION, 0);
            if (mModel != null && mModel.recurrenceState != RecurrenceModel.STATE_NO_RECURRENCE) {
            ...
            if (b != null) {
                ...
                /**
                 * M: get the event time duration.
                 */
                mTimeDuration = b.getLong(BUNDLE_EVENT_TIME_DURATION, 0);
        ...
        /** M: findbugs fix, STYLE : DLS_DEAD_LOCAL_STORE.*/
        ...
        /*
         * M: re-calculate the repeat rules to allow to select.
         */
        CharSequence[] repeatArray = getActivity().getResources().getTextArray(
                R.array.recurrence_freq);
        int minRepeatMode = getMinRepeatMode(mTimeDuration);
        if (minRepeatMode > 0) {
            repeatArray = Arrays.copyOfRange(repeatArray,
                    (minRepeatMode - EventRecurrence.DAILY),
                    (EventRecurrence.YEARLY - EventRecurrence.DAILY + 1));
        }
        ArrayAdapter<CharSequence> freqAdapter = new ArrayAdapter<CharSequence>(
                getActivity(), R.layout.recurrencepicker_freq_item, repeatArray);
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        /**
         * M: save the event time duration.
         */
        outState.putLong(BUNDLE_EVENT_TIME_DURATION, mTimeDuration);
    }
    ...
    public void updateDialog() {
        ...
        /*
         * M: adjust the selection position according to minimum repeat mode
         * which was allowed.
         */
        int minRepeatMode = getMinRepeatMode(mTimeDuration);
        int selection = mModel.freq;
        if (minRepeatMode > 0) {
            selection -= minRepeatMode - EventRecurrence.DAILY;
        }
        mFreqSpinner.setSelection(selection);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        if (parent == mFreqSpinner) {
            ...
            /*
             * M: add the offset to the position to get the correct recurrence
             * frequency.
             */
            int minRepeatMode = getMinRepeatMode(mTimeDuration);
            if (minRepeatMode > 0) {
                mModel.freq += minRepeatMode - EventRecurrence.DAILY;
            }
    ...
    /**
     * M: Using event time duration to find out the minimum repeat mode cycle
     * that we can use.
     * for example: duration is 2 days, then return EventRecurrence.WEEKLY
     */
    public static int getMinRepeatMode(long duration) {
