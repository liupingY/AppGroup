...
//    public static final int LAST_CALENDAR_DAY = 2465059; /* 2036.12.31 in Julian */
    public static final int LAST_CALENDAR_DAY = 2465424; /* 2037.12.31 in Julian */
    ///@}
    ...
    **/
    ...
    public static String getRingTonePreference(Context context) {
        ...
        if (ringtone == null) {	//prize-public-bug:19187 set ringtone to null -pengcancan-20160805-start
        ...
        setRingTonePreference(context, ringtone);//prize-public-bug:19187 set ringtone to null -pengcancan-20160805-end
        return ringtone;
    }
    ...
    /** M: add needMoveToFirst parameter. */
    public static MatrixCursor matrixCursorFromCursor(Cursor cursor, boolean needMoveToFirst) {
        if (cursor == null) {
            return null;
        }
        ...
        /** M: move to first pos before return cursor if need.@{ */
        if (needMoveToFirst) {
...
//        Intent launchIntent = new Intent(context, PrizeSettingsActivity.class);
...
//        launchIntent.putExtra(INTENT_KEY_HOME, true);
     ...
     */
    public static void setMonthTitle(LayerDrawable icon, Context c, int month) {
    	MonthDrawable monthDrawable;

        // Reuse current drawable if possible
        Drawable currentDrawable = icon.findDrawableByLayerId(R.id.title);
        if (currentDrawable != null && currentDrawable instanceof MonthDrawable) {
        	monthDrawable = (MonthDrawable)currentDrawable;
        } else {
        	monthDrawable = new MonthDrawable(c);
        }
        monthDrawable.setMonth(month);
        icon.mutate();
        icon.setDrawableByLayerId(R.id.title, monthDrawable);
    }
     ...
     * @param icon - today's icon from the options menu
     */
    public static Drawable setTodayIcon(Drawable icon, Context c, String timezone) {
        DayOfMonthDrawable today;
        Drawable currentDrawable = icon;
        if (currentDrawable != null && currentDrawable instanceof DayOfMonthDrawable) {
            today = (DayOfMonthDrawable)currentDrawable;
        } else {
            today = new DayOfMonthDrawable(c);
    ...
    public static boolean isFirstLunarDay(String lunarDay){
        boolean isFirstDay = false;
        if((lunarDay.equals("一月"))||(lunarDay.equals("二月"))||(lunarDay.equals("三月"))
                ||(lunarDay.equals("四月"))||(lunarDay.equals("五月"))||(lunarDay.equals("六月"))
                ||(lunarDay.equals("七月"))||(lunarDay.equals("八月"))||(lunarDay.equals("九月"))
                ||(lunarDay.equals("十月"))||(lunarDay.equals("十一月"))||(lunarDay.equals("十二月"))
                ||(lunarDay.equals("闰一"))||(lunarDay.equals("闰二"))||(lunarDay.equals("闰三"))
                ||(lunarDay.equals("闰四"))||(lunarDay.equals("闰五"))||(lunarDay.equals("闰六"))
                ||(lunarDay.equals("闰七"))||(lunarDay.equals("闰八"))||(lunarDay.equals("闰九"))
                ||(lunarDay.equals("闰十"))||(lunarDay.equals("闰十一"))||(lunarDay.equals("闰十二"))
                /*||(lunarDay.equals("元旦"))*/||(lunarDay.equals("春节"))){
            isFirstDay= true;
        }
        return isFirstDay;
    }
