...
public class AgendaWindowAdapter extends BaseAdapter
    ...
    private final String MTAG = "LIXING";
    ...
    /** prize-adjust event layout-wanzhijuan-2015-7-8-start */
    private final TextView mHeaderTextView;
    private final TextView mFooterTextView;
    /** prize-adjust event layout-wanzhijuan-2015-7-8-end */
    private boolean mDoneSettingUpHeaderFooter = false;
    /// M: add CT new feature, Agenda view without event. @{
    ...
    /**
     * M: the header view generally include header and footer view, and also include
     * no event item view when there is no event in the list view.
     */
    private static final int HEADER_VIEW_MAX_COUNT = 1;
    ...
    public static void setTopDeviation(int[] topDeviationInfo) {
        if (topDeviationInfo == null || topDeviationInfo.length != 2) {
            return;
        }
        sTopDeviationInfo[0] = topDeviationInfo[0];
        sTopDeviationInfo[1] = topDeviationInfo[1];
    }
    /// @}

    /*
     * M: Save the top deviation from AgendaFragment onSaveInstanceState, this
     * will be used to make the switch screen action looks smoothly.
     */
    public int[] saveTopDeviation(Time time, long eventId) {
        /*
         * Get the first visible position, if it is 0, it is the list header;
         * if getItemViewType(firstVisiblePosition - 1) is 0, it is a DAYTYPE
        ...
        int firstItemOnSameTime = findEventPositionNearestTime(time, eventId) + 1;
        int itemNum = firstVisiblePosition - firstItemOnSameTime;

        Log.i(TAG, "time=" + time + ", eventId=" + eventId + ", firstVisiblePosition="
                + firstVisiblePosition + ", firstItemOnSameTime="
                + firstItemOnSameTime + ", itemNum=" + itemNum
    ...
    static class DayAdapterInfo {
        ...
        public DayAdapterInfo(Context context, boolean isMonth) {
            dayAdapter = new AgendaByDayAdapter(context,isMonth);
        }
    ...
    public AgendaWindowAdapter(Context context,
            AgendaListView agendaListView, boolean showEventOnStart, boolean isMonth) {
        mContext = context;
        ...
        /** prize-adjust event layout-wanzhijuan-2015-7-8-start */
        mHeaderView = inflater.inflate(R.layout.agenda_header, null);
        mHeaderTextView = (TextView) mHeaderView.findViewById(R.id.tv_content);
        
        mFooterView = inflater.inflate(R.layout.agenda_footer, null);
        mFooterTextView = (TextView) mFooterView.findViewById(R.id.tv_content);
        
        mHeaderTextView.setText(R.string.loading);
        /** prize-adjust event layout-wanzhijuan-2015-7-8-end */
        
        /// M: add CT new feature, Agenda view without event, set blank text
        mBlankView = (LinearLayout) inflater.inflate(R.layout.agenda_blank, null);
        TextView tView = (TextView) mBlankView.findViewById(R.id.blank_text);
        tView.setText(mContext.getString(R.string.no_event_tip));
...
//        mAgendaListView.addHeaderView(mHeaderView);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (!isMonthAgenda && position >= (mRowCount - PREFETCH_BOUNDARY)
                && mNewerRequests <= mNewerRequestsProcessed) {
        ...
        if (!isMonthAgenda && position < PREFETCH_BOUNDARY
                && mOlderRequests <= mOlderRequestsProcessed) {
    ...
    public AgendaItem getAgendaItemByPosition(final int positionInListView,
        ...
        /**prize-calendar-hekeyi-2015.5.11-begin*/
//        final int positionInAdapter = positionInListView - OFF_BY_ONE_BUG;
        final int positionInAdapter;        
        if(!isMonthAgenda){
        	positionInAdapter = positionInListView - OFF_BY_ONE_BUG;
        }else{
        	positionInAdapter = positionInListView;
        }
        /**prize-calendar-hekeyi-2015.5.11-end*/
    ...
    public void refresh(Time goToTime, long id, String searchQuery, boolean forced,
    	...
    	/*t f  f*/
    	if(goToTime != null){
    		Log.d(MTAG,"in AgendaWindowAdapter.refresh() goToTime = " + goToTime.monthDay + ":" + goToTime.hour + ":" + goToTime.minute + " ,searchQuery = " + searchQuery    			
    			+ ",id = "+ id +",forced = " + forced + ",refreshEventInfo = " + refreshEventInfo + ",isMonth = " + isMonth);/*t f f*/
    	}else{
    		Log.d(MTAG,"in AgendaWindowAdapter.refresh() goToTime = null" + " ,searchQuery = " + searchQuery    			
        ...
        Log.d(MTAG,"AgendaWindowAdapter.refresh().startDay is:" + startDay );
        

        if (!forced && isInRange(startDay, startDay)) {
        	
        	Log.d(MTAG,"forced =" + forced + ",startDay is:" + startDay );
        ...
        Log.d(MTAG,"mCleanQueryInitiated =" + mCleanQueryInitiated + ",searchQuery = " +searchQuery);
    ...
    private synchronized boolean queueQuery(int start, int end, Time goToTime,
...
//            }
    ...
    private void doQuery(QuerySpec queryData) {
        ...
        Log.d(MTAG,"queryData.start = "+ queryData.start + " ,queryData.end = " + queryData.end + ",queryData.searchQuery = " + queryData.searchQuery 
        		+ ",queryData.id = " + queryData.id + "queryData.queryStartMillis" + queryData.queryStartMillis );
    ...
    private void updateHeaderFooter(final int start, final int end) {
		isAddFlag = false;
        mHeaderTextView.setText(mContext.getString(R.string.show_older_events,
                formatDateString(start)));
        mFooterTextView.setText(mContext.getString(R.string.show_newer_events,
    ...
    private class QueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            if (cursor == null) {
                LogUtil.e(TAG, "onQueryComplete, cursor is null.");
 				/** prize-Adjust list layout-wanzhijuan-2015-7-8-start */
                if (mHeaderTextView != null) {
                    mHeaderTextView.setText(R.string.loading_failed);
                }
 				 /** prize-Adjust list layout-wanzhijuan-2015-7-8-end */
                return;
            }
            ///@}
            ...
            Log.d(MTAG, "onQueryComplete, data.queryType = " + data.queryType );
            if (data.queryType == QUERY_TYPE_CLEAN) {           
                mCleanQueryInitiated = false;
            }
            Log.d(MTAG, "onQueryComplete, mCleanQueryInitiated = " + mCleanQueryInitiated);
            ...
            /// M: add CT new feature, Agenda view without event. @{
            int tempCursorSize = cursor.getCount();
            int totalCount = mAgendaListView.getCount();
            
            /*PRIZE-update-wanzhijuan-2015-6-29-start*/
            if (0 == tempCursorSize && isMonthAgenda && !mHasBlankView && mAgendaListView.getHeaderViewsCount() == 0) {
            	Log.d(TAG,"mAgendaListView.getCount() is:" + totalCount +"cursor.getCount() is:" + tempCursorSize);
                mAgendaListView.addHeaderView(mBlankView);
                mHasBlankView = true;
            } else {
                if ((0 < tempCursorSize && isMonthAgenda) && mHasBlankView && mAgendaListView.getHeaderViewsCount() > 0) {
                    mAgendaListView.removeHeaderView(mBlankView);
                    mHasBlankView = false;
                    Log.d(TAG,"mAgendaListView.getCount() is:" + totalCount +"cursor.getCount() is:" + tempCursorSize);
                }
            }
            ...
            if (!mDoneSettingUpHeaderFooter && !isMonthAgenda) {
            ...
            if (cursorSize > 0 || mAdapterInfos.isEmpty() || data.queryType == QUERY_TYPE_CLEAN) {
                ...
                if (data.goToTime == null) { // Typical Scrolling type query
                    ...
                    if (newPosition >= 0) {
                        ...
                        Log.d(TAG,
                                "real postion: "
                                        + (newPosition + sTopDeviationInfo[0] + OFF_BY_ONE_BUG)
                                        + ", top deviation: "
                                        + sTopDeviationInfo[1]
                        ...
                        if(actualTime.toMillis(false)==CalendarController.getInstance(mContext).getTime()){
                ...
                /// M: For tablet with event detail in agenda view.
                //  In agenda view, after clear event and back to agenda view,
                //  both event list and event info view need refresh.
                //  Below is for refresh event info view in case of clear event then back. @{
                boolean showEventDetailsWithAgenda =
                        Utils.getConfigBool(mContext, R.bool.show_event_details_with_agenda);
                if (mIsTabletConfig && showEventDetailsWithAgenda && cursorSize == 0
                        && !(mContext instanceof com.mediatek.calendar.selectevent.AgendaChoiceActivity)) {
                    if (DEBUGLOG) {
                        Log.d(TAG, "onQueryComplete: Tablet platform and cursor size is 0, need update event view.");
                    }
                    Time current = new Time(mTimeZone);
                    current.setToNow();
                    AgendaItem item = new AgendaItem();
                    item.id = -1;
                    item.begin = current.toMillis(true);
                    item.end = current.toMillis(true);;
                    item.startDay = Time.getJulianDay(current.toMillis(true), current.gmtoff);
                    item.allDay = false;
                    sendViewEvent(item, current.toMillis(true));
                }
                /// @}
            } else {
                cursor.close();
            ...
            synchronized (SLOCK) {
                ...
                if (cursorSize != 0) {
                    ...
                    if (BASICLOG &&x != null &&  !x.equals(data)) {
                    ...
                    if (!mAdapterInfos.isEmpty()&&querySpec!=null) {
                ...
                if(mQueryQueue.size()==0&&isAddFlag){
        ...
        private int processNewCursor(QuerySpec data, Cursor cursor) {
            synchronized (mAdapterInfos) {
                ...
                if (info == null) {
                    info = new DayAdapterInfo(mContext, isMonthAgenda);
                } else {
                    if (DEBUGLOG)
