 ...
 */
package com.prize.autotest.horcali;
...
import com.prize.autotest.horcali.AFMFunctionCallEx;
import com.prize.autotest.horcali.FunctionReturn;
...
public class SensorCalibration extends Activity implements OnClickListener, SensorEventListener {
            ...
            public void handleMessage(Message msg) {
				...
				case MSG_UI_UPDATE:
					...
					if(mSuccess){
						...
						Log.e("zwl", "mUiHandler times = " + times);
						if(times < 2){
							times++;
	...
	protected void onResume() {
		...
		if (mSensor != null) {
			...
			Toast.makeText(this, getSensorName(mType) + " was not supported.", Toast.LENGTH_SHORT).show();
			setResult(RESULT_CANCELED);
			finish();
		}
		...
		getCalibration();
		Log.e("zwl", "onResume() times = " + times);
        times++;
		clearGsensorCalibration();
		mHandler.sendEmptyMessage(MSG_DO_CALIBRARION_20);
		mHorizontalCalibrationView.isCalibrating = true;
		mCalibrationNote.setText(getResources().getString(R.string.calibration_attention_calibrating));
		mUiHandler.post(HandlerThread);
	}
	...
	private boolean getCalibration() {
		...
		Log.d(TAG, String.format("getGsensorCalibration(), ret %d, values %f, %f, %f", ret, result[0], result[1],
				result[2]));

		if (ret == RET_SUCCESS) {
			// mData = String.format(Locale.ENGLISH, "%+8.4f,%+8.4f,%+8.4f",
			// result[0], result[1], result[2]);
			mData = String.format(Locale.ENGLISH, "%+d,%+d,%+d", (int) (result[0] * 1000), (int) (result[1] * 1000),
					(int) (result[2] * 1000));
	...
	public int clearGsensorCalibration() {
		String[] ret = runCmdInEmSvr(AFMFunctionCallEx.FUNCTION_EM_SENSOR_CLEAR_GSENSOR_CALIBRATION, 0);
	...
	public int getGsensorCalibration(float[] result) {
		String[] ret = runCmdInEmSvr(AFMFunctionCallEx.FUNCTION_EM_SENSOR_GET_GSENSOR_CALIBRATION, 0);
	...
	public int doGsensorCalibration(int tolerance) {
		String[] ret = runCmdInEmSvr(AFMFunctionCallEx.FUNCTION_EM_SENSOR_DO_GSENSOR_CALIBRATION, 1, tolerance);
		if (ret.length > 0 && String.valueOf(RET_SUCCESS).equals(ret[0])) {
			return RET_SUCCESS;
		}
		return RET_FAILED;
	...
	public void onSensorChanged(SensorEvent event) {
		...
		case Sensor.TYPE_ACCELEROMETER:
			...
			float mDefaultDistance = (float) (mHorizontalCalibrationView.background.getWidth() / 2 * 0.8);
	...
	Runnable HandlerThread = new Runnable() {
		@Override
		public void run() {
			if (mHorizontalCalibrationView.radius > mHorizontalCalibrationView.bubble.getWidth() * 0.9 / 2) {
				mHorizontalCalibrationView.radius = mHorizontalCalibrationView.radius - mReduction;
	...
	void resetForCircle() {
		mUiHandler.removeCallbacks(HandlerThread);
		mCalibrationNote.setText(getResources().getString(R.string.calibration_attention_init));
		mHorizontalCalibrationView.radius = (float) (mHorizontalCalibrationView.background.getWidth() * 0.9 / 2);
