...
public class TPTestSurfaceView extends SurfaceView implements Callback {
	...
	public int mModeType = TP_TEST_SENSITIVITY;// First view.
	Context mContext;
	...
	private void setupAccuracy() {
		...
		arrayAccuracyCircles = new int[][] {
				new int[] { WIDTH_ACCURACY_RADIUS1, WIDTH_ACCURACY_RADIUS1,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED },
				new int[] { width / 2, WIDTH_ACCURACY_RADIUS1,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED },
				new int[] { width - WIDTH_ACCURACY_RADIUS1,
						WIDTH_ACCURACY_RADIUS1, WIDTH_ACCURACY_RADIUS1,
						PAINT_COLOR_RED },
				new int[] { width / 4, height / 4, WIDTH_ACCURACY_RADIUS2,
						PAINT_COLOR_RED },
				new int[] { width * 3 / 4, height / 4, WIDTH_ACCURACY_RADIUS2,
						PAINT_COLOR_RED },
				new int[] { WIDTH_ACCURACY_RADIUS1, height / 2,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED },
				new int[] { width / 2, height / 2, WIDTH_ACCURACY_RADIUS2,
						PAINT_COLOR_RED },
				new int[] { width - WIDTH_ACCURACY_RADIUS1, height / 2,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED },
				new int[] { width / 4, height * 3 / 4, WIDTH_ACCURACY_RADIUS2,
						PAINT_COLOR_RED },
				new int[] { width * 3 / 4, height * 3 / 4,
						WIDTH_ACCURACY_RADIUS2, PAINT_COLOR_RED },
				new int[] { WIDTH_ACCURACY_RADIUS1,
						height - WIDTH_ACCURACY_RADIUS1,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED },
				new int[] { width / 2, height - WIDTH_ACCURACY_RADIUS1,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED },
				new int[] { width - WIDTH_ACCURACY_RADIUS1,
						height - WIDTH_ACCURACY_RADIUS1,
						WIDTH_ACCURACY_RADIUS1, PAINT_COLOR_RED }, };
	...
	private void setupSensitivity() {
		...
		arraySensitivityPoints = new int[][] {
				...
				new int[] { width - widthSensitivity * 2,
						height - widthSensitivity * 2 },
	...
	private void setupZoom() {
		...
		arrayZoomCircles = new int[][] {
				...
				new int[] { width / 2, height / 2, width * 4 / 10,
						PAINT_COLOR_WHITE },
	...
	private void setupLinearityHorizontal() {
		...
		int middleHeight = height
				- (heightLinearityHorizontalYBorder * 2 * scale);
		int heightLinearityHorizontalY = middleHeight / (5 * scale);

		arrayLinearityHorizontalY = new int[][] {
				new int[] {
						heightLinearityHorizontalYBorder * 1
								+ heightLinearityHorizontalY * 0,
						PAINT_COLOR_GREEN },
				new int[] {
						heightLinearityHorizontalYBorder * 2
								+ heightLinearityHorizontalY * 1,
						PAINT_COLOR_GREEN },
				new int[] {
						heightLinearityHorizontalYBorder * 2
								+ heightLinearityHorizontalY * 3,
						PAINT_COLOR_GREEN },
				new int[] {
						heightLinearityHorizontalYBorder * 2
								+ heightLinearityHorizontalY * 5,
						PAINT_COLOR_GREEN },
				new int[] {
						heightLinearityHorizontalYBorder * 2
								+ heightLinearityHorizontalY * 7,
						PAINT_COLOR_GREEN },
				new int[] {
						heightLinearityHorizontalYBorder * 2
								+ heightLinearityHorizontalY * 9,
						PAINT_COLOR_GREEN },
				new int[] {
						heightLinearityHorizontalYBorder * 3
								+ heightLinearityHorizontalY * 10,
						PAINT_COLOR_GREEN } };
	...
	private void initRedPaint2() {
		if (null == mRedPaint2) {
			...
			setPaintBaseAttribute(mRedPaint2, Color.argb(80, red, green, blue),
					mPaintWidth2);
		}
	}
	...
	private void initRedPaint3() {
		if (null == mRedPaint3) {
			...
			int green = Color.green(Color.RED);
			int blue = Color.blue(Color.RED);
			setPaintBaseAttribute(mRedPaint3, Color.argb(80, red, green, blue),
					mPaintWidth2_border);
		}
	}
	...
	private void initGreenPaint2() {
		if (null == mGreenPaint2) {
			...
			setPaintBaseAttribute(mGreenPaint2,
					Color.argb(80, red, green, blue), mPaintWidth2);
		}
	}
	...
	private void initGreenPaint3() {
		if (null == mGreenPaint3) {
			...
			int green = Color.green(Color.GREEN);
			int blue = Color.blue(Color.GREEN);
			setPaintBaseAttribute(mGreenPaint3,
					Color.argb(80, red, green, blue), mPaintWidth2_border);
		}
	}
	...
	private void initBluePaint2() {
		if (null == mBluePaint2) {
			...
			setPaintBaseAttribute(mBluePaint2,
					Color.argb(150, red, green, blue), PAINT_WIDTH_3);
	...
	private void setPaintBaseAttribute(Paint paint, int color, int width) {
		// Anti-aliasing.
		paint.setAntiAlias(true); // Paint paint = new
	...
	private void handleEventLinearity(MotionEvent event, int i) {
		...
		case TP_TEST_LINEARITY1:
			...
			case MotionEvent.ACTION_MOVE:
				if (isDrawing) {
					float calculatedX = currentY * (getWidth() - marginX)
							/ getHeight();
					if (currentX < calculatedX + LINEARITY_WIDTH
							&& currentX > calculatedX - LINEARITY_WIDTH + 20) {
						if (currentX >= getWidth() - LINEARITY_WIDTH
								&& currentY >= getHeight() - LINEARITY_WIDTH) {
							bLinearity1 = true;
							toNext();
						} else {
		...
		case TP_TEST_LINEARITY2:
			...
			case MotionEvent.ACTION_MOVE:
				if (isDrawing) {
					float calculatedX = getWidth()
							- (currentY * (getWidth() - marginX) / getHeight())
							- marginX;
					if (currentX < calculatedX + LINEARITY_WIDTH
							&& currentX > calculatedX - LINEARITY_WIDTH + 20) {
						if (currentX <= LINEARITY_WIDTH
								&& currentY >= getHeight() - LINEARITY_WIDTH) {
							bLinearity2 = true;
	...
	private void handleEventAccuracy(MotionEvent event) {
		...
		if (null != arrayAccuracyCircles && arrayAccuracyCircles.length > 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				double distance = Math.sqrt(Math.abs(currentX - centerX)
						* Math.abs(currentX - centerX)
						+ Math.abs(currentY - centerY)
						* Math.abs(currentY - centerY));
				if (distance <= radius) {
	...
	private void handleEventSensitivity(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			clearTouchPath();
			if (currentX <= mWidthSensitivity
					&& currentY >= getHeight() - mWidthSensitivity) {
				drawTouchDown(currentX, currentY);
			}
			invalidate();
			break;
		case MotionEvent.ACTION_MOVE:
			if (isDrawing) {
				...
				case SENSITIVITY_BORDER_1:
					...
					if (currentX <= arraySensitivityPoints[1][0]
							&& currentY <= arraySensitivityPoints[1][1]) {
				...
				case SENSITIVITY_BORDER_2:
					...
					if (currentX >= arraySensitivityPoints[2][0]
							&& currentY <= arraySensitivityPoints[2][1]) {
				...
				case SENSITIVITY_BORDER_3:
					...
					if (currentX >= arraySensitivityPoints[3][0]
							&& currentY >= arraySensitivityPoints[3][1]) {
						mBorderType = SENSITIVITY_BORDER_4;
					}
					break;
				case SENSITIVITY_BORDER_4:// point 1 and point 5
					if (currentX >= arraySensitivityPoints[0][0]
							&& currentY >= arraySensitivityPoints[4][1]) {
						isAllowCrossBorder = true;
					} else {
						isAllowCrossBorder = false;
					}
					if (currentX >= arraySensitivityPoints[0][0]
							&& currentX <= arraySensitivityPoints[4][0]
							&& currentY >= arraySensitivityPoints[4][1]) {
				...
				case SENSITIVITY_BORDER_5:// point 2 and point 6
					if (currentX >= arraySensitivityPoints[1][0]
							&& currentX <= arraySensitivityPoints[5][0]
							&& currentY >= arraySensitivityPoints[1][1]) {
						isAllowCrossBorder = true;
					} else {
						isAllowCrossBorder = false;
					...
					if (currentX >= arraySensitivityPoints[1][0]
							&& currentX <= arraySensitivityPoints[5][0]
							&& currentY >= arraySensitivityPoints[1][1]
							&& currentY <= arraySensitivityPoints[5][1]) {
						mBorderType = SENSITIVITY_BORDER_6;
					}
					break;
				case SENSITIVITY_BORDER_6:// point 3 and point 7
					if (currentX >= arraySensitivityPoints[1][0]
							&& currentX <= arraySensitivityPoints[2][0]
							&& currentY >= arraySensitivityPoints[2][1]
							&& currentY <= arraySensitivityPoints[6][1]) {
						isAllowCrossBorder = true;
					} else {
						isAllowCrossBorder = false;
					}
					if (currentX >= arraySensitivityPoints[6][0]
							&& currentX <= arraySensitivityPoints[2][0]
							&& currentY >= arraySensitivityPoints[2][1]
							&& currentY <= arraySensitivityPoints[6][1]) {
				...
				case SENSITIVITY_BORDER_7:// point 4 and point 8
					if (currentX >= arraySensitivityPoints[7][0]
							&& currentX <= arraySensitivityPoints[3][0]
							&& currentY >= arraySensitivityPoints[2][1]
							&& currentY <= arraySensitivityPoints[3][1]) {
						isAllowCrossBorder = true;
					} else {
					...
					if (currentX >= arraySensitivityPoints[7][0]
							&& currentX <= arraySensitivityPoints[3][0]
							&& currentY >= arraySensitivityPoints[7][1]
							&& currentY <= arraySensitivityPoints[3][1]) {
						mBorderType = SENSITIVITY_BORDER_8;
					}
					break;
				case SENSITIVITY_BORDER_8:// point 5 and point 9
					if (currentX >= arraySensitivityPoints[4][0]
							&& currentX <= arraySensitivityPoints[3][0]
							&& currentY >= arraySensitivityPoints[8][1]
							&& currentY <= arraySensitivityPoints[4][1]) {
						isAllowCrossBorder = true;
					} else {
						isAllowCrossBorder = false;
					...
					if (currentX >= arraySensitivityPoints[4][0]
							&& currentX <= arraySensitivityPoints[8][0]
							&& currentY >= arraySensitivityPoints[8][1]
							&& currentY <= arraySensitivityPoints[4][1]) {
				...
				case SENSITIVITY_BORDER_9:// point 6 and point 10
					if (currentX >= arraySensitivityPoints[5][0]
							&& currentX <= arraySensitivityPoints[9][0]
							&& currentY >= arraySensitivityPoints[5][1]
							&& currentY <= arraySensitivityPoints[4][1]) {
						isAllowCrossBorder = true;
					} else {
				...
				case SENSITIVITY_BORDER_10:// point 7 and point 10
					...
					if (currentX >= arraySensitivityPoints[9][0]
							&& currentX <= arraySensitivityPoints[6][0]
							&& currentY >= arraySensitivityPoints[6][1]
							&& currentY <= arraySensitivityPoints[9][1]) {
				...
				case SENSITIVITY_BORDER_11:// point 8 or point 9
					if (currentX >= arraySensitivityPoints[8][0]
							&& currentX <= arraySensitivityPoints[7][0]
							&& currentY >= arraySensitivityPoints[6][1]
							&& currentY <= arraySensitivityPoints[7][1]) {
						isAllowCrossBorder = true;
					} else {
					...
					if (currentX >= arraySensitivityPoints[8][0]
						...
						isAllowCrossBorder = false;
						bSensitivity = true;
						AutoTPTestActivity.isTPResult = true;
						toNext();
					}
					break;
				default:
	...
	private void handleEventZoom(MotionEvent event) {
		if (event.getPointerCount() == 2) {
			...
			double distance = Math.sqrt(Math.abs(currentX1 - currentX2)
					* Math.abs(currentX1 - currentX2)
					+ Math.abs(currentY1 - currentY2)
					* Math.abs(currentY1 - currentY2));
			...
			case MotionEvent.ACTION_MOVE:
				if (mPreviousDistance != 0) {
					...
					// to next.
					if (arrayZoomCircles[0][3] == PAINT_COLOR_GREEN
							&& arrayZoomCircles[2][3] == PAINT_COLOR_GREEN) {
						bZoom = true;
						toNext();
					}
	...
	private void handleEventLinearityHorizontal(MotionEvent event) {
		...
		int measureWidth = 0;
		if (mCurrentLinearityHorizontalPosition == 0
				|| mCurrentLinearityHorizontalPosition == mLinearityHorizontalCount - 1) {
		...
		case MotionEvent.ACTION_DOWN:
			clearTouchPath();
			if (currentY <= currentPositionY + measureWidth / 2
					&& currentY >= currentPositionY - measureWidth / 2) {
		...
		case MotionEvent.ACTION_MOVE:
			if (isDrawing) {
				if (currentY <= currentPositionY + measureWidth / 2
					...
					//AutoTPTestActivity.isTPResult = false;
					isDrawing = false;
	...
	private void handleEventLinearityVertical(MotionEvent event) {// Vertical
		...
		case MotionEvent.ACTION_DOWN:
			clearTouchPath();
			if (currentX <= currentPositionX + mPaintWidth2 / 2
					&& currentX >= currentPositionX - mPaintWidth2 / 2) {
	...
	private void drawTouchDown(float currentX, float currentY) {
		...
		mInvalidRect.set((int) currentX, (int) currentY, (int) currentX,
				(int) currentY);
		mCurveEndX = currentX;
		mCurveEndY = currentY;
	}
	...
	private void drawTouchMove(float currentX, float currentY) {
		...
		if (dx >= 3 || dy >= 3) {
			...
			areaToRefresh.union((int) mPreviousX, (int) mPreviousY,
					(int) mPreviousX, (int) mPreviousY);
			areaToRefresh.union((int) mCurveEndX, (int) mCurveEndY,
					(int) mCurveEndX, (int) mCurveEndY);
	...
	private void drawLinearity(Canvas canvas, int type) {
		...
		if (type == TP_TEST_LINEARITY1) {
			canvas.drawLine((float) LINEARITY_WIDTH, 0.0f, (float) width,
					(float) (height - LINEARITY_WIDTH), mRedPaint);
			canvas.drawLine(0.0f, (float) LINEARITY_WIDTH,
					(float) (width - LINEARITY_WIDTH), (float) height,
					mRedPaint);
			return;
		} else if (type == TP_TEST_LINEARITY2) {
			canvas.drawLine((float) (width - LINEARITY_WIDTH), 0.0f, 0.0f,
					(float) (height - LINEARITY_WIDTH), mRedPaint);
			canvas.drawLine((float) width, (float) LINEARITY_WIDTH,
					(float) LINEARITY_WIDTH, (float) height, mRedPaint);
		}
	}
	...
	private void drawAccuracy(Canvas canvas) {
		...
		int count = arrayAccuracyCircles.length;
		for (int i = 0; i < count; i++) {
			canvas.drawCircle(arrayAccuracyCircles[i][0],
					arrayAccuracyCircles[i][1], arrayAccuracyCircles[i][2],
					arrayAccuracyCircles[i][3] == PAINT_COLOR_RED ? mRedPaint
							: mGreenPaint);
		}
	}
	...
	private void drawSensitivity(Canvas canvas) {
		...
		for (int i = 0; i < count - 1; i++) {
			canvas.drawLine(arraySensitivityPoints[i][0],
					arraySensitivityPoints[i][1],
					arraySensitivityPoints[i + 1][0],
					arraySensitivityPoints[i + 1][1], mRedPaint);
		}
	}
	...
	private void drawZoom(Canvas canvas) {
		...
		for (int i = 0; i < count; i++) {
			...
			canvas.drawCircle(arrayZoomCircles[i][0], arrayZoomCircles[i][1],
					arrayZoomCircles[i][2], p);
		}
	}
