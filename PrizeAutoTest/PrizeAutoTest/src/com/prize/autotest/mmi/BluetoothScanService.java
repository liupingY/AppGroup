	...
	BroadcastReceiver mBluetoothDeviceReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothDevice.ACTION_FOUND.equals(action)) {
				Log.e(TAG, "BluetoothScanService ACTION_FOUND size ="
						+ mDeviceList.size());
				Log.e(TAG,
						"BluetoothScanService" + " name =" + device.getName()
								+ " addr = " + device.getAddress());
				mDeviceList.add(new DeviceInfo(device.getName(), device
						.getAddress(), intent.getExtras().getShort(
						BluetoothDevice.EXTRA_RSSI)));
				app.setBluetoothDeviceList(mDeviceList);
			} else if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED
					.equals(action)) {
				Log.e(TAG,
						"BluetoothScanService ACTION_DISCOVERY_FINISHED size ="
								+ mDeviceList.size());
				...
				Log.e(TAG,
						"BluetoothScanService ACTION_DISCOVERY_STARTED size ="
								+ mDeviceList.size());
			} else if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
				Log.e(TAG, "BluetoothScanService ACTION_STATE_CHANGED size ="
						+ mDeviceList.size());
				if (BluetoothAdapter.STATE_ON == intent.getIntExtra(
					...
					if (recordTime) {
						...
						.getIntExtra(BluetoothAdapter.EXTRA_STATE, 0)) {
					Log.e(TAG, "BluetoothScanService STATE_TURNING_ON size ="
							+ mDeviceList.size());
				} else if (BluetoothAdapter.STATE_TURNING_OFF == intent
						.getIntExtra(BluetoothAdapter.EXTRA_STATE, 0)) {
					Log.e(TAG, "BluetoothScanService STATE_TURNING_OFF size ="
							+ mDeviceList.size());
				}
			}
