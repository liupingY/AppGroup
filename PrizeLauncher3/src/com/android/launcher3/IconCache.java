...
public class IconCache {
    ...
    private CacheEntry cacheLocked(ComponentName componentName, ResolveInfo info,
        ...
        if (entry == null) {
            ...
            Log.i("oyj","cacheLocked componentName : "+componentName);
            if(LqShredPreferences.isLqtheme(mContext)){
            	Bitmap bitmap = LqService.getInstance().getIcon(componentName, null, true, "");
            		 entry.icon = bitmap;
            }
 ...
 // Added by lqsoft STSRT   *******************************************************
    public Bitmap getCustomIcon(ShortcutInfo item){
        ...
        Bitmap b = null;
		if (LqShredPreferences.isLqtheme(mContext)) {
	         b = LqService.getInstance().getIcon(null, icon, true, "");
		}
        return b;
    }
    ...
    public Bitmap getResourcesIcon(String packageName, String resourceName,boolean  processed) {
		...
		if(processed){
			   Bitmap icon =null;
			if (LqShredPreferences.isLqtheme(mContext)) {
	             icon = LqService.getInstance().getIcon(null, bp, true, "");
			}
            return icon;
		}
		return bp;
