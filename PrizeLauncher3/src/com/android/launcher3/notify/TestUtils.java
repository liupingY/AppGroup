...
public class TestUtils {
    ...
    public static Object getSuperClassProperty(Object owner, String fieldName) {
        Object property = null;
        try {
            Class ownerClass = owner.getClass().getSuperclass(); //得到超类对象  BaseStatusBar
            Field field = ownerClass.getDeclaredField(fieldName); //通过BaseStatusBar得到类声明的属性  mCommandQueue
            field.setAccessible(true); //设置可访问所有属性 public，protected private
            property = field.get(ownerClass);  //通过对象得到该属性的实例  mCommandQueue  这里报错了 why
        } catch (IllegalArgumentException e) {
            Log.d(TAG, "IllegalArgumentException fieldName = " + fieldName); 
            /*java.lang.IllegalArgumentException: expected receiver of type com.android.systemui.statusbar.BaseStatusBar, 
            but got java.lang.Class<com.android.systemui.statusbar.BaseStatusBar>*/
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            Log.d(TAG, "IllegalAccessException fieldName = " + fieldName);
        } catch (NoSuchFieldException e) {
