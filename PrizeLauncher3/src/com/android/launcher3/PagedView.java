...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public  final static int UNITIS = 2200;
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        if (xMoved || xPaged || yMoved) {
            if (mUsePagingTouchSlop ? xPaged : xMoved) {
				...
				//add by zhouerlong
                if(isFlingingToShowSysteUI() == FLING_STATE.DOWN) {

                    mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
                    //add by zhouerlong
//                    mOnlyFirstExcute = true;
                    mFlingState = FLING_STATE.DOWN;
                }else if (isFlingingToShowSysteUI() == FLING_STATE.UP) {
                    mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
...
//            	mTouchYState = TOUCH_STATE_SCROLLING;
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if(isSupportCycleSlidingScreen()&&this instanceof Workspace) {
    ...
    protected void  handleEventDist(MotionEvent ev) {
                	if (this.mIsCanPerformeTowFinger) {
                		...
                		if (newMotionEventDist <this.mLastMotionPointdist) {
	 ...
	 * @param ev
	 */
	private void handleMaskPointUpEvent(MotionEvent ev) {
        ...
        final float x = ev.getX(pointerIndex);//当前滑动的x坐标
...
//        final float x = ev.getX(pointerIndex);
        final float deltaX = DELTAX_VELOCITYTRACKER_MAX*(mLastMotionX + mLastMotionXRemainder - x);
