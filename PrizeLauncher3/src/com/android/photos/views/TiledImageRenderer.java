...
public class TiledImageRenderer {
    ...
    private void layoutTiles() {
        ...
        synchronized (mQueueLock) {
            ...
            for (int i = 0; i < n; i++) {
                Tile tile = mActiveTiles.get(i);
                int level = tile.mTileLevel;
                if (level < fromLevel || level >= endLevel
                        || !range[level - fromLevel].contains(tile.mX, tile.mY)) {
                    mActiveTiles.remove(i);
                    i--;
                    n--;
    ...
    public void freeTextures() {
        ...
        int n = mActiveTiles.size();
        for (int i = 0; i < n; i++) {
            Tile texture = mActiveTiles.get(i);
					//add by zhouerlong begin 20150811
            if (texture != null) {
                texture.recycle();
            }
    ...
    private void uploadBackgroundTiles(GLCanvas canvas) {
        ...
        for (int i = 0; i < n; i++) {
            Tile tile = mActiveTiles.get(i);
            if (tile !=null &&!tile.isContentValid()) {
                queueForDecode(tile);
            }
        }
    }
   ...
   private void queueForDecode(Tile tile) {
    ...
    private void recycleTile(Tile tile) {
        synchronized (mQueueLock) {
            if (tile != null && tile.mTileState == STATE_DECODING) {
