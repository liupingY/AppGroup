...
public class RegActivity extends BaseActivity {
	...
	private void setAgree() {
		mAgree.setCompoundDrawablesWithIntrinsicBounds(R.drawable.agree, 0,
				0, 0);
			...
			public void onClick(View widget) {
				if (isAgreed)
					mAgree.setCompoundDrawablesWithIntrinsicBounds(
							R.drawable.register_not_agree, 0, 0, 0);
				else
					mAgree.setCompoundDrawablesWithIntrinsicBounds(
							R.drawable.agree, 0, 0, 0);
				isAgreed = !isAgreed;
				enableNext();
	...
	private void toCode(String account) {
		...
		//it.putExtra(CodeActivity.PARAM_TITLE, "注册酷比账号");
		Log.e("huang", "cccccc");
		startActivity(it);
	}
	...
	private void enableNext() {
		...
		if (isAgreed && Utils.isPhone(text) || Utils.isEmail(text)) {
			mNextBtn.setBackgroundResource(R.drawable.next_selector);
