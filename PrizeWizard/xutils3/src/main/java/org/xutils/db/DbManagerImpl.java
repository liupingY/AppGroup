...
public final class DbManagerImpl implements DbManager {
	...
	public void saveOrUpdate(Object entity) throws DbException {
		try {
			...
			if (entity instanceof List) {
				...
				TableEntity<?> table = TableEntity.get(this, entities.get(0)
						.getClass());
			...
			if (entity instanceof List) {
				...
				for (Object item : entities) {
					execNonQuery(SqlInfoBuilder
							.buildReplaceSqlInfo(table, item));
				}
			} else {
				TableEntity<?> table = TableEntity.get(this, entity.getClass());
				createTableIfNotExist(table);
		...
		try {
			...
			if (entity instanceof List) {
				...
				if (!table.tableIsExist())
					return;
				for (Object item : entities) {
					execNonQuery(SqlInfoBuilder.buildUpdateSqlInfo(table, item,
							updateColumnNames));
				}
			} else {
				TableEntity<?> table = TableEntity.get(this, entity.getClass());
	...
	public <T> T findById(Class<T> entityType, Object idValue)
		...
		Selector selector = Selector.from(table).where(
				table.getId().getColumnName(), "=", idValue);
	...
	// *****************************
	private void saveOrUpdateWithoutTransaction(TableEntity<?> table,
			Object entity) throws DbException {
	...
	private boolean saveBindingIdWithoutTransaction(TableEntity<?> table,
			Object entity) throws DbException {
		ColumnEntity id = table.getId();
		if (id.isAutoId()) {
			execNonQuery(SqlInfoBuilder.buildInsertSqlInfo(table, entity));
		...
		try {
			...
			if (col != null) {
				...
				builder.append("ALTER TABLE ").append("\"")
						...
						.append(" ADD COLUMN ").append("\"")
						.append(col.getColumnName()).append("\"").append(" ")
						.append(col.getColumnDbType()).append(" ")
						.append(col.getProperty());
	...
	public void execNonQuery(SqlInfo sqlInfo) throws DbException {
		/*
		 * try { Object[] bindArgs = sqlInfo.getBindArgs(); if (bindArgs != null
		 * && bindArgs.length > 0) { database.execSQL(sqlInfo.getSql(),
		 * bindArgs); } else { database.execSQL(sqlInfo.getSql()); } } catch
		...
		try {
			...
			IOUtil.closeQuietly((Closeable) statement);
