...
import com.prize.runoldtest.util.BatteryUtil;
...
public class DeviceInfoWindowService extends Service {
	...
	public void onCreate() {
		...
		// mCPUHeaderText.append("CPU LOADER" + LF);
		// mCPUCoreNum = CpuUtil.getNumCores();
		// mCPUHeaderText.append("CPU Number:" + mCPUCoreNum + LF);
		// mCPUHeaderText.append("1min\t5min\t15min\tCurrent" + LF);
		...
		mPowerHeaderText.append("Battery State" + LF);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case HANDLE_CHECK_ACTIVITY:
				...
				mIsPowerDisp = mSP.getBoolean(getString(R.string.battery_key), true);
				StringBuffer sb = new StringBuffer();

				// if (mIsCpuDisp) {
				// sb.append(mCPUHeaderText.toString());
				// sb.append(CpuUtil.getCpuName() + LF);
				/*
				 * for (int i = 0; i < mCPUCoreNum; i++) {
				 * sb.append(String.format(eachCpuInfo, i, cpuMaxList.get(i),
				 * cpuMinList.get(i), getKHz(CpuUtil.getCurCpuFreq(i)))); }
				 */
				// }
				if (mIsPowerDisp && (BatteryT / 10 < 40)) {
					sb.append(mPowerHeaderText);
					sb.append("Current    :" + BatteryUtil.getCurrent() + LF);
					// sb.append("Voltage :" + BatteryV/1000 + "V" + LF);
					sb.append("Temperature:" + BatteryT / 10 + "℃" + LF);
					sb.append("Quantity   :" + BatteryN + "%" + LF);
				} else {
					SystemProperties.set(Const.SYSPROPERTY_RUN_TEST_CPU, Const.RUNIN_TEST_FAIL + "");
				}
				...
				mHandler.sendEmptyMessageDelayed(HANDLE_CHECK_ACTIVITY, REFRESH_TIME);
				break;
			}
		}
	};
	...
	private BroadcastReceiver mBatInfoReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_BATTERY_CHANGED.equals(action)) {
				...
				BatteryT = intent.getIntExtra(BatteryManager.EXTRA_TEMPERATURE, 0); // 电池温度
			}
		}
	};
}
