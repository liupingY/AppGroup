...
public class Utils {
	...
	public static final String AUTHORITY = "com.prize.appcenter.provider.appstore"; //com.prize.left.page.provider.login
	
	public static final Uri PERSON_URI = Uri.parse("content://" + AUTHORITY + "/table_person");
	...
	public static void saveInfo(Context ctx, LoginInfo info) {
		ctx.getContentResolver().insert(PERSON_URI, info2Values(info));
		/*DBApplication.getInstance().getDatabase().insert("table_person", null, info2Values(info));*/
	}
	...
	public static void saveAccount(Context ctx, ContentValues values) {
		Uri uri = Uri.parse("content://" + AUTHORITY + "/table_account");
		ctx.getContentResolver().insert(uri, values);
		uri = null;
		// DBApplication.getInstance().getDatabase().insert("table_account", null, values);
	}
	...
	public static CloudAccount curAccount(Context ctx) {
		...
		// Cursor cs = DBApplication.getInstance().getDatabase().query("table_account", null, null, null, null, null, null);
	...
	public static Person getPersonalInfo(Context ctx) {
		Uri uri = Uri.parse("content://" + AUTHORITY + "/table_person");
		Cursor cs = ctx.getContentResolver().query(uri, null, null, null, null);
		//Cursor cs = DBApplication.getInstance().getDatabase().query("table_person", null, null, null, null, null, null);
		Person person = null;
		if (cs != null && cs.moveToFirst()) {
			person = new Person();
	 ...
	 * @return
	 */
	public static int updateEmail(Context ctx, String email) {//, String userId
		Uri uri = Uri.parse("content://" + AUTHORITY + "/email");
		
		ContentValues values = new ContentValues();
		values.put("email", email);
		// return DBApplication.getInstance().getDatabase().update("table_person", values, "userId=?", 
		// 		new String[] {userId});
	...
	public static void logout(Context ctx) {
		...
		//DBApplication.getInstance().getDatabase().delete("table_person", null, null);
		uri = Uri.parse("content://" + AUTHORITY + "/table_account");
		ctx.getContentResolver().delete(uri, null, null);
		//DBApplication.getInstance().getDatabase().delete("table_account", null, null);
