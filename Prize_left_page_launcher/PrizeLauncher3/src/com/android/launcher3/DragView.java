...
public class DragView extends View {
	...
	public void bringToFront() {
		LogUtils.i("zhouerlong", "bringToFront");
		super.bringToFront();
	}
    ...
    public DragView(Launcher launcher, Bitmap bitmap, int registrationX, int registrationY,
        ...
        if(launcher.getDragController().mDragObject!=null) {
             dragInfo = launcher.getDragController().mDragObject.dragInfo;
        }
        ...
        mAnim.setDuration(Utilities.getDragDuration());
    ...
    public void showNoAnimation(int touchX, int touchY) { //将其add 到DragLayer中
    	if (mDragLayer.indexOfChild(this) ==-1) {
    		LogUtils.i("zhouerlong", "addddddddddddddd111111111111");
            mDragLayer.addView(this);
     ...
     * @param touchY the y coordinate the user touched in DragLayer coordinates
     */
    void move(int touchX, int touchY,VelocityTracker v) {
        ...
        if(v!=null) {
        	...
        	LogUtils.i("zhouerlong", "xV:"+xV+" yV:"+yV);
            setRotationX(getRotX(yV));
            setRotationY(getRotY(xV));
//            touchX+=getVelOffX(xV);
//            touchY += getVelOffY(yV);
        }
        setTranslationX(touchX - mRegistrationX + (int) mOffsetX);
        setTranslationY(touchY - mRegistrationY + (int) mOffsetY);
        /// M: @}
    }
    private final float MAXVEL=700f;
    
    private float getRotX(float vel) {
    	float result = 0;
    		vel = Math.max(Math.min(vel, MAXVEL), -MAXVEL);
    		 result = vel/MAXVEL*45;
    		 LogUtils.i("zhouerlong", "result::vel"+result);
    			
    	return result;
    }
    
    private float getRotY(float vel) {
    	float result = 0;
    		vel = Math.max(Math.min(vel, MAXVEL), -MAXVEL);
    		 result = -vel/MAXVEL*45;
    		 LogUtils.i("zhouerlong", "result::vel"+result);
    ...
    private float getVelOffY(float vel) {
    	float result = 0;
    		vel = Math.max(Math.min(vel, MAXVEL), -MAXVEL);
    		 result = vel/MAXVEL*50;
    		 Log.i("zhouerlong", "result::vel"+result);
    			
    	return result;
    }
    
    private float getVelOffX(float vel) {
    	float result = 0;
    		vel = Math.max(Math.min(vel, MAXVEL), -MAXVEL);
    		 result = vel/MAXVEL*50;
    		 LogUtils.i("zhouerlong", "result::vel"+result);
    ...
    void remove() {
        ...
        LogUtils.i("zhouerlong", "remove DragView: this = " + this);
