...
import com.android.appwidget.AppWidgetHostView;
...
//            throw new RuntimeException("All Device Profiles must have an odd number of hotseat spaces");
        ...
        numColumns = c;//列数
        iconSize = is;//图标大小
        iconTextSize = its;//图标文本大小
        numHotseatIcons = hs;//hotseat图标个数
        hotseatIconSize = his;//hotseat图标大小
    }
    ...
    DeviceProfile(Context context,
        ...
        defaultWidgetPadding = com.android.appwidget.AppWidgetHostView.getDefaultPaddingForWidget(context, cn, null);
        ...
        int icon_size =Utilities.getDefaultIconSize((int)iconSize);
        iconSizePx = (int) (Launcher.scale*icon_size);//DynamicGrid.pxFromDp(iconSize, dm);
        ...
        hotseatIconSizePx = (int) (Launcher.scale*icon_size);//DynamicGrid.pxFromDp(iconSize, dm);
        hotseatAllAppsRank = 3;//(int) (numColumns / 2);
        ...
        textPaint.setTextSize(Launcher.textSize);
        FontMetrics fm = textPaint.getFontMetrics();
        cellWidthPx = iconSizePx;
        cellHeightPx = (int) (iconSizePx + (int) Math.ceil(fm.bottom - fm.top)+9/2*Launcher.scale);
        ...
        */
        ...
        if (AppsCustomizePagedView.DISABLE_ALL_APPS) {
                hotseatBarHeightPx = iconSizePx + 8 * edgeMarginPx;
        }
        hotseatCellWidthPx = iconSizePx;
        hotseatCellHeightPx = iconSizePx;

        // Folder
        folderCellWidthPx = (int) (cellWidthPx + 2.8f * edgeMarginPx*context.getResources().getDimension(R.dimen.folder_cell_w));//???cell??
        folderCellHeightPx = (int) (cellHeightPx + (int) ((3f/2f) * edgeMarginPx)*context.getResources().getDimension(R.dimen.folder_cell_h));//???cell??
        folderBackgroundOffset = -edgeMarginPx+edgeMarginPx;
        folderIconSizePx = iconSizePx /*+ 2 * -folderBackgroundOffset*/;//????icon????
    }
    ...
    private float invDistWeightedInterpolate(float width, float height,
        ...
        Collections.sort(pointsByNearness, new Comparator<DeviceProfileQuery>() {//比较查看是否和最相近的匹配
    ...
    Rect getWorkspacePadding(int orientation) {
        ...
        if (orientation == CellLayout.LANDSCAPE &&
            ...
            if (isTablet()) {
            	...
            	int dim=1;
            	if (Launcher.viewHeight==960) {
            		dim=2;
            	}else if (Launcher.viewHeight==854) {

            		dim=3;
            	}else if (Launcher.viewHeight==800) {
            		dim=0;
            	}
            	if (AppsCustomizePagedView.DISABLE_ALL_APPS) {
                    padding.set(desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.left,
                            searchBarSpaceHeightPx+edgeMarginPx*dim,
                            desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.right,
                            hotseatBarHeightPx -edgeMarginPx*15+ pageIndicatorHeightPx);
            	}else {
                    padding.set(desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.left,
                            searchBarSpaceHeightPx,
                            desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.right,
...
//        View voiceButtonProxy = launcher.findViewById(R.id.voice_button_proxy);
     /*   if (voiceButtonProxy != null) {
        ...
        if (hasVerticalBarLayout) {
            ...
            Rect pad = getWorkspacePadding(isLandscape
                    ? CellLayout.LANDSCAPE
                    : CellLayout.PORTRAIT);
            lp.gravity = Gravity.BOTTOM;
            lp.width = LayoutParams.MATCH_PARENT;
            lp.height = hotseatBarHeightPx;
            hotseat.findViewById(R.id.layout).setPadding(pad.left, 0,
            		pad.right, 0);
        }
...
public class DynamicGrid {
    ...
    public DynamicGrid(Context context, Resources resources,
        ...
        deviceProfiles.add(new DeviceProfile("Super Short Stubby",
        		/*最小宽度*/255, /*最小高度*/300,  /*行*/2, /*列*/3,  /*图标大小*/48, /*图标文本大小*/13, /*hotseart个数*/(hasAA ? 5 : 4), /*hetseart图标大小*/48)); //最小宽度，最小高度 行数，列数，图标大小 ，图标文本大小 hotseat图标个数，hotseat图标大小
        deviceProfiles.add(new DeviceProfile("Shorter Stubby",
                255, 400,  3, 3,  65, 13, (hasAA ? 4 : 4), 65));
        deviceProfiles.add(new DeviceProfile("Short Stubby",
                275, 420,  3, 4,  65, 13, (hasAA ? 4 : 4), 65));
        deviceProfiles.add(new DeviceProfile("Stubby",
                255, 450,  3, 4,  65, 13, (hasAA ? 4 : 4), 65)); //hdpi 480*800
        deviceProfiles.add(new DeviceProfile("Nexus S",
                296, 491.33f,  5, 4,  45, 13, (hasAA ? 4 : 4), 45));
        /// M: add 294 X 460 profile
        deviceProfiles.add(new DeviceProfile("294 X 460",
                294, 460,  4, 4,  50.34943f, 13, (hasAA ? 4 : 4), 49.566288f));
	//add by bxh
		if (Launcher.isSupportGridChange) {
			if (Launcher.currentLayoutGrid == 20) {
				deviceProfiles.add(new DeviceProfile("Nexus 4", 359, 518, 5, 4,
						65, 13, (hasAA ? 4 : 4), 65));// add by zhouerlong
			} else {

				deviceProfiles.add(new DeviceProfile("Nexus 4", 359, 518, 4, 4,
						65, 13, (hasAA ? 4 : 4), 65));// add by zhouerlong
			}
		} else {
        if (AppsCustomizePagedView.DISABLE_ALL_APPS) {
                deviceProfiles.add(new DeviceProfile("Nexus 4",
                        359, 518,  5, 4,  85, 13, (hasAA ? 4 : 4), 55));//add by zhouerlong
        }else {

            deviceProfiles.add(new DeviceProfile("Nexus 4",
                    359, 518,  4, 4,  65, 13, (hasAA ? 4 : 4), 65));//add by zhouerlong
        }
       }
			//A by zel
						//A by zel
