...
public class WallpaperCropActivity extends Activity {
    ...
    private static  float WALLPAPER_SCREENS_SPAN = 2f;

    protected static CropView mCropView;
    protected Uri mUri;

    //xieweiwei_20150205_add_begin
    //it will be set in the derived class WallpaperPickerActivity
    protected static boolean mIsSetLockscreenWallpaper = false;
    //xieweiwei_20150205_add_end
	...
	//add b zel
    synchronized protected void OnSetWallpaperFinish() {// m b zel ????wallpaper
    ...
    protected void init() {
        ...
        if (Launcher.isScrollWallpaper) {
        	this.WALLPAPER_SCREENS_SPAN = 2.0f;
        }else {
        	this.WALLPAPER_SCREENS_SPAN = 1.0f;
        }
	...
	//????????? ????
    static float cropMaxScreenSpan(int inwidth,int inheight,Context context) {

		Display display = ((Activity) context).getWindowManager().getDefaultDisplay();
		int defualtwidth = display.getWidth();
		int defaultheight = display.getHeight();
    		if ((inwidth/(float)inheight)>=(defualtwidth/(float)defaultheight*2f)) {
    			return 2.0f;
    		}else  {
    			return 1.0f;
    		}
    }
    static protected Point getDefaultWallpaperSize(Resources res, WindowManager windowManager,float wallpaper_screen_span) {
        ...
        if (isScreenLarge(res)) {
        	...
        	if (Launcher.isScrollWallpaper) {
                defaultWidth = Math.max((int) (minDim * wallpaper_screen_span), maxDim);
        	}else {
                defaultWidth = Math.max((int) (minDim * wallpaper_screen_span), minDim);
        	}
            defaultHeight = maxDim;
        }
    ...
    private static int getRotationFromExifHelper(
        ...
        try {
            ...
            Log.w(LOGTAG, "security exception: ", e);
        }
        return 0;
    }
    ...
    protected void cropImageAndSetWallpaper(
        ...
        Point outSize = getDefaultWallpaperSize(getResources(),
                getWindowManager(),cropMaxScreenSpan(inSize.x,inSize.y,this));
...
//                    finish();//?á?é¨o¨a|ì??a¨¤?¨o??a¨ｿ2?¨¨?launcher ¨|¨¨???à¨2???¨a¨a?3?
//del by zhouerlong
                }
            }
        };
        ...
        if (isScreenLarge(getResources())) {
        	...
        	if (Launcher.isScrollWallpaper) {
                defaultWallpaperWidth = Math.max((int)
                        (minDim * cropMaxScreenSpan(inSize.x, inSize.y, mCropView.getContext())), maxDim);
        	}else {
                defaultWallpaperWidth = Math.max((int)
                        (minDim * cropMaxScreenSpan(inSize.x, inSize.y, mCropView.getContext())), minDim);
        	}
        }
...
//        Point inSize = mCropView.getSourceDimensions();
        ...
        private void regenerateInputStream() {
            if (mInUri == null && mInResId == 0 && mInFilePath == null && mInImageBytes == null) {
                ...
                try {
                    if (mInUri != null) {
                        /// M: DRM file
                        if (FeatureOption.MTK_DRM_APP && BitmapRegionTileSource.isDrmFormat(mContext, mInUri) == true) {
                            ...
                            if(filePath != null) {
                                ...
                                if(bitmap != null) {
                                    mInStream = BitmapRegionTileSource.getByteArrayInputStream(bitmap);
                                } else {
        ...
        public boolean cropBitmap() {
            ...
            if (mSetWallpaper && mNoCrop && mInStream != null) {
                ...
                if (mIsSetLockscreenWallpaper) {
                    Utilities.setFlagOfSettingLockscreenWallpaper(mContext.getContentResolver(), true);
                } else {
                    Utilities.setFlagOfSettingLockscreenWallpaper(mContext.getContentResolver(), false);
                }
                //xieweiwei_20150205_add_end
            ...
            if (mInStream != null) {
                ...
                if (crop == null) {
                    ...
                    if (fullSize != null) {
                        /// M: scaleDownSampleSize can't be zero
    ...
    static public void suggestWallpaperDimension(Resources res,
            ...
            final WallpaperManager wallpaperManager,final boolean isLauncherSet) {
    	 Point inSize = mCropView.getSourceDimensions();
        final Point defaultWallpaperSize = getDefaultWallpaperSize(res, windowManager,cropMaxScreenSpan(inSize.x,inSize.y,mCropView.getContext())); 
        new Thread("suggestWallpaperDimension") {
            public void run() {
                // If we have saved a wallpaper width/height, use that instead
				Bitmap b = wallpaperManager.getBitmap();
				Display display = ((Activity) mCropView.getContext())
				...
				if (b != null) {
					...
					if ((sw / (float) sh) >= (defualtwidth
							/ (float) defaultheight * 2f)&& !isLauncherSet) {

						SharedPreferences.Editor editor = sharedPrefs.edit();
						editor.putInt(WALLPAPER_WIDTH_KEY,
								defaultWallpaperSize.x * 2);
						editor.putInt(WALLPAPER_HEIGHT_KEY,
								defaultWallpaperSize.y);
						savedHeight = sharedPrefs.getInt(WALLPAPER_HEIGHT_KEY,
								defaultWallpaperSize.y);
						editor.commit();
					}
				}

				savedWidth = sharedPrefs.getInt(WALLPAPER_WIDTH_KEY,
