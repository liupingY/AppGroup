...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    protected void onFinishInflate() {
        ...
        mUninstallDrawable = (TransitionDrawable) 
                mContext.getDrawable(R.drawable.uninstall_target_selector);
        mRemoveDrawable = (TransitionDrawable) mContext.getDrawable(R.drawable.remove_target_selector);
    ...
    public static boolean willAcceptDrop(Object info) { if (info instanceof ItemInfo) {
            ItemInfo item = (ItemInfo) info;


        	List appList = Utilities.getActiveAppList(LauncherApplication.getInstance().getApplicationContext());

            if(appList != null && info instanceof ShortcutInfo){
        		if( appList.contains(((ShortcutInfo)info).packageName))
        			return false;
        	}

            if(info instanceof ShortcutInfo&&Launcher.isSupportClone) {
            	if(/*((ShortcutInfo)info).container == LauncherSettings.Favorites.CONTAINER_HOTSEAT ||*/   /// @lixing 注释掉此句,修复第三方应用在hotseat上拖拽无卸载框出现
            			((ShortcutInfo)info).intent!=null&&((ShortcutInfo)info).intent.getAppInstanceIndex() == 1) {
            		return false;
            	}
            }
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
    	List appList = Utilities.getActiveAppList(mContext);
//    	((ResolveInfo)appList.get(0)).activityInfo.packageName
        ...
        if(mLauncher.getWorkspace().getMultipleDragViews().size()>0) {
            isVisible = false;
        }
        ...
        mActive = isVisible;
        resetHoverColor();
        ((ViewGroup) getParent().getParent().getParent()).setVisibility(isVisible ? View.VISIBLE : View.GONE);
		...
		final ItemInfo item = (ItemInfo) d.dragInfo;
		 ...
		 if(item instanceof LauncherAppWidgetInfo) {
			 ...
			 if(Launcher.isSupportTranslateByUninstall) {
					ValueAnimator update =  ValueAnimator.ofFloat(1,0f);
					update.addUpdateListener(new AnimatorUpdateListener() {
						
						@Override
						public void onAnimationUpdate(ValueAnimator animation) {
							float percent = (float) animation.getAnimatedValue();
							mLauncher.updateTranslationYbyExplosionView(percent,false,true);
						}
					});
					update.setDuration(600);
					update.start();
			 }
		 }else {
			 if(isUninstallFromWorkspace(d)) {
                ...
                if (mWaitingForUninstall) {
                    ...
                    final Runnable checkIfUninstallWasSuccess = new Runnable() {
                        ...
                        public void run() {
                            ...
                            LogUtils.i("zhouerlong", "uninstallSuccessful:::"+uninstallSuccessful+" mWaitingForUninstall::"+mWaitingForUninstall);
//                            if(uninstallSuccessful) {
                            	try {
//                            		mLauncher.getworkspace().mDeferDropAfterUninstall=false;
//                                	mLauncher.getWorkspace().removeViewByComponentName(mLauncher.getWorkspace().getDragInfo().cell);
								} catch (Exception e) {
...
//                            }
                            if (dragSource instanceof Folder) {
                                ((Folder) dragSource).
                                    onUninstallActivityReturned(mWaitingForUninstall);
                            } else if (dragSource instanceof Workspace&&!mWaitingForUninstall) {
                    ...
                    checkIfUninstallWasSuccess.run();
//                    mLauncher.addOnResumeCallback(checkIfUninstallWasSuccess);
                }
            }
        } else if (isWorkspaceOrFolderApplication(d)) {
        	try {
            	mLauncher.getModel().getDownLoadService().pikerRemoveCurrentTask(item.packageName);
			} catch (Exception e) {
				e.printStackTrace();
			}
