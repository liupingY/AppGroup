...
//    private AppFilter mAppFilter;
    ...
    public void add(AppInfo info) {
        ...
        if(mFilterApps==null) {
            mFilterApps =Utilities.getFilterApps();
        }
        
        		

		try {
        if(info.componentName!=null&&mFilterApps!=null) {
        	if(!info.componentName.getPackageName().equals("com.android.launcher3")&&!mFilterApps.contains(info.componentName.getPackageName())){
        		if(info.componentName.getPackageName().equals("com.prize.factorytest")) {
        			boolean factorytest=true;
        			try {
        				factorytest=!readProInfo(46).equals("P");
					} catch (Exception e) {
						factorytest=true;
					}
        			if(factorytest) {
	...
	//add by zhouerlong
	public void removeSTKActivity() {
		  this.removePackage("com.android.stk");
		  this.removePackage("com.mediatek.StkSelection");
     ...
     */
    public void updatePackage(Context context, String packageName,boolean isFromAppStore) {
        final List<ResolveInfo> matches = findActivitiesForPackage(context, packageName);
        if (LauncherLog.DEBUG) {
        ...
        if (matches.size() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                LogUtils.i("zhouerlong", "applicationInfo::::"+applicationInfo);
                if (applicationInfo == null&&!isFromAppStore) {
                    add(new AppInfo(context.getPackageManager(), info, mIconCache, null));
                } else {
                	if (isFromAppStore) {
                		AppInfo appInfo =	new AppInfo(context.getPackageManager(), info, mIconCache, null);
                        modified.add(appInfo);
                        LogUtils.i("zhouerlong", "test");
                	}else {
    ...
    static boolean loadTopPackage(final Context context) {
        ...
        try {
            XmlResourceParser parser = context.getResources().getXml(R.xml.default_toppackage1);
