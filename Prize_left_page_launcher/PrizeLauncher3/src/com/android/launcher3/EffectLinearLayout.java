...
public class EffectLinearLayout extends LinearLayout implements
	...
	private ItemInfo layout_5;
	private ItemInfo layout_4;
	...
	//add by bxh	
		if(Launcher.isSupportGridChange){
			layout_5 = new ItemInfo();
			layout_5.icon = this.getContext().getResources()
					.getDrawable(R.drawable.normals);
			layout_5.title = this.getContext().getString(R.string.Layout_5);
			
			layout_4 = new ItemInfo();
			layout_4.icon = this.getContext().getResources()
					.getDrawable(R.drawable.normals);
			layout_4.title = this.getContext().getString(R.string.Layout_4);
		}
...
//		themes.add(random);//
	...
	public void setCurrentSelect() {
        ...
        int d= Utilities.getDefaultEffct();
		View child = this.getChildAt(PreferencesManager.getSharedPreferences(
				this.getContext()).getInt(
				PreferencesManager.KEY_GENERAL_EFFECT_INDEX, d));
		if (child == null) {
			child = this.getChildAt(d);
		}
	...
	public void onClick(View view) {
		...
		if (mLauncher.isSupportGridChange) {
			if (index > 1) {
