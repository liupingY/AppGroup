...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    private static final float DELTAX_VELOCITYTRACKER_MAX = 1.6f; 
	//add by zhouerlong
    ...
    public  final static int UNITIS = 1500;
...
//        }
	...
	public void showList() {
		...
		show.setOnPauseListener(new PauseOnListener(ImageLoader.getInstance(),true));
		if (this instanceof AppsCustomizePagedView) {
			show.start((ViewGroup) this.getChildAt(this.getChildCount() - 1
					- mCurrentPage),true);
		} else {
			show.start((ViewGroup) this.getChildAt(mCurrentPage),true);
		}
	}
	...
	public void backList() {
		...
		if (this instanceof AppsCustomizePagedView) {
			...
			show.start((ViewGroup) this.getChildAt(mCurrentPage),false);
		}
    ...
    public void scrollTo(int x, int y) {
        ...
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "scrollTo: x = " + x + ", y = " + y + ", mOverScrollX = " + mOverScrollX + ", mMaxScrollX = "
                    + mMaxScrollX + ", mScrollX = " + mScrollX + ",mUnboundedScrollX = " + mUnboundedScrollX + ", this = "
                    + this);
        }
        mUnboundedScrollX = x;
        mUnboundedScrollY = y;
...
//                super.scrollTo(0, y);
        	...
        	if(mMotionEvent != null&&mMotionEvent.getPointerCount()<=1) {
        		if(mContext instanceof Launcher) {
        			...
        			/*if(l.getScrollLayout().isSupportCycleSlidingScreen()) {
                    	requestDisallowInterceptTouchEventByScrllLayout(false);
        			}*/
        		}
        	}
...
//        lp.isFullScreenPage = true;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(Launcher.isSupportLeftnavbar) {
                    childHeight = heightSize - verticalPadding - mInsets.top - 0;
				...
				if(child instanceof LeftFrameLayout) {
					...
					if(Launcher.isSupportLeftnavbar) {
						 childHeight = heightSize - mInsets.bottom;/*- verticalPadding - mInsets.top - mInsets.bottom*/
					}else {
						 childHeight = heightSize;/*- verticalPadding - mInsets.top - mInsets.bottom*/
					}
				}
...
//                        	Log.i("zhouerlong", "ForceDrawAllChildrenNextFrame:"+"["+leftScreen+","+rightScreen+"]"+"i:"+i);
	...
	//add by zhouerlong 
        case MotionEvent.ACTION_POINTER_DOWN:
    		...
    		Rect r = new Rect();
        	mLastMotionPointdist = this.spacing(ev,r);
...
//            	mOnlyFirstExcute = false;//add by zhouerlong
            ...
            case MotionEvent.ACTION_UP:
				//add by zhouerlong
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "onInterceptTouchEvent up touch down:   "
                            +   ", mScrollX = " + mScrollX + ", mTouchState = " + mTouchState + ", this = " + this);
                }
                if(!((Launcher)mContext).getworkspace().isLeftFrame()&&getChildCount()>1) {
                    handleMaskPointUpEvent(ev,true);
                }
	...
	//add by zhouerlong 
        ...
        return mTouchState != TOUCH_STATE_REST||mTouchYState != TOUCH_STATE_REST;
    }
...
//    private boolean mOnlyFirstExcute=false;
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        boolean xPaged = xDiff > mPagingTouchSlop; //滑动页数最小距离

        boolean xMoved = xDiff > touchSlop;//判断是否触发滑动
        boolean yMoved = yDiff > touchSlop/2;//判断Y轴是否触发滑动
...
//                    mOnlyFirstExcute = true;
                	...
                	this.handleEventDist(ev);//A by zel
                	if(ev.getPointerCount()>1) {
                        mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
                	}else {
            ...
            if (yMoved) {
//                mTouchState = TOUCH_STATE_REST;
//            	mTouchYState = TOUCH_STATE_SCROLLING;
            }//A by zel
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if(isSupportCycleSlidingScreen()&&(this instanceof Workspace|| this instanceof PrizeScrollLayout)) {
...
//            		Log.i("zhouerlong", "screenCenter: "+screenCenter+" getScrollX():"+getScrollX());
        ...
        float scrollProgress = delta / (totalDistance * 1f);
        if (LauncherLog.DEBUG_DRAW) {
     ...
     * @return
     */
    protected float spacing(MotionEvent event,Rect r) {
    	         float x = 0 ;
    	         float y = 0 ;
    	         try {  
    	        	 if (event.getPointerCount() >= 2){
    	        		  x = event.getX() - event.getX(1);  
    	    	          y = event.getY() - event.getY(1);
    	        	 }
    	             r.left = (int) event.getRawX();
    	             r.top = (int) event.getRawY();
    	             r.right= (int) (event.getRawX()+x);
    	             r.bottom = (int) (event.getRawY()+y);
    ...
    protected void  expandSystemUI() {
		  ((Launcher)this.getContext()).expand();
    }
    
    /**两手指收缩或者张开 进入编辑模式或者退出编辑模式
     * @param ev
     */
    protected void  handleEventDist(MotionEvent ev) {
                	if (this.mIsCanPerformeTowFinger) {
                		Rect r = new Rect();
                		float newMotionEventDist = this.spacing(ev,r);
                		if (newMotionEventDist <this.mLastMotionPointdist&&mLastMotionPointdist>getViewportWidth()/2) {
                			if (this instanceof Workspace) {
                				Workspace w = (Workspace) this;
                				if (!w.isInSpringLoadMoed() && !w.isLeftFrame()) {
                					if (this.getContext() instanceof Launcher) {
                						Launcher l = (Launcher) this.getContext();
                						/*
                						 * 进入编辑模式
                						 */

                	                    mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
                            			w.enterSprindLoadMode(l.getCurrrentSpringState());
                	                    mFlingState = FLING_STATE.MULTI_DOWN;
                		...
                		/*else if (newMotionEventDist>this.mLastMotionPointdist){
                			if (this instanceof Workspace) {
                				Workspace w = (Workspace) this;
                				if (w.isInSpringLoadMoed()) {
            						Launcher l = (Launcher) this.getContext();
            						 
//                        			w.exitSpringLoadMode(true,false);
                        			
                				}
                				if(Launcher.isSupportHideApp) {
        	                    mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
	...
	private void handleMaskPointUpEvent(MotionEvent ev,boolean intercept) {
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "Touch up scroll: x = " + x + ", deltaX = " + deltaX
                    + ", mTotalMotionX = " + mTotalMotionX + ", mLastMotionX = "
		//add by zhouerlong
                    + mLastMotionX + ", velocityY = " + velocityY+", velocityX = " + velocityX + ", mCurrentPage = "
        ...
        if (!mFreeScroll) {
            ...
            if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
              ...
              if(mFlingState !=FLING_STATE.UP&&mFlingState!=FLING_STATE.DOWN)
                ...
                if(mCurrentPage ==getPageCount()-1 && mScrollX<0){
                	if(getChildCount()<=1) {
...
//        final float x = ev.getX(pointerIndex);
		...
		if(this instanceof Workspace) {
			deltaX= DELTAX_VELOCITYTRACKER_MAX*(mLastMotionX + mLastMotionXRemainder - x);
			if(this.getChildCount()<=1) {
				deltaX= (mLastMotionX + mLastMotionXRemainder - x);
			}
		}
...
//        ((Launcher) getContext()).onClick(this);
    ...
    int getPageNearestToCenterOfScreen() {
		...
		if(this instanceof Workspace){
			if(((Workspace)this).isLeftFrame()) {
				m = 0.5f;
				ms = 1 -m;
			}
		}
        int screenCenter = getViewportOffsetX() + getScrollX() + (int)(getViewportWidth() *0.5);
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; ++i) {
            View layout = (View) getPageAt(i);
            int childWidth = layout.getMeasuredWidth();
            int halfChildWidth =(int) (childWidth *0.5);
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (isSupportCycleSlidingScreen() && ((mCurrentPage == 0 && whichPage == getChildCount() - 1)
            ...
            if (newX == 0 && (getScrollX() <= 0 || getScrollX() >= mMaxScrollX)) {
				...
				LauncherLog.d("yhf","snapToPageWithVelocity,newx=0 && ..,mUnboundedScrollX="+mUnboundedScrollX);
            }
    ...
    protected void snapToPage(int whichPage, int duration, boolean immediate) {
        ...
        LogUtils.i("zhouerlong", "whichPage   "+whichPage);
...
//                delta += getViewportWidth();//add by zel
				...
				LauncherLog.d("yhf","snapToPage,newX == 0 && ...,mUnboundedScrollX="+mUnboundedScrollX);
            }
        }
