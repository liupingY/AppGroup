...
public class PagedViewWidget extends LinearLayout {
    ...
    public void applyFromAppWidgetProviderInfo(AppWidgetProviderInfo info,
        ...
        if(isGroup) {
        	try {
                ResolveInfo  rs = getActivitisForPkg(info.providerInfo.packageName);
                if (rs!=null) {
                	title = (String) rs.loadLabel(mContext.getPackageManager())+"("+groupSize+")";					
				}
			} catch (Exception e) {
				e.printStackTrace();
    ...
    private ResolveInfo getActivitisForPkg(String pkg) {
		...
		List<ResolveInfo> appList = mContext.getPackageManager()
    ...
    public void applyFromResolveInfo(
        ...
        if(isGroup) {
        	try {
                ResolveInfo  rs = getActivitisForPkg(info.activityInfo.packageName);
                label = rs.loadLabel(mContext.getPackageManager())+"("+groupSize+")";
			} catch (Exception e) {
				e.printStackTrace();
			}
        }
        ...
        if(label.toString().contains("News")) {
            name.setText(label);
        }
        name.setText(label);
        final TextView dims = (TextView) findViewById(R.id.widget_dims);
    ...
    void applyPreview(Drawable preview, int index) {
        ...
        if (preview != null) {
           ...
           /* if (AppsCustomizePagedView.DISABLE_ALL_APPS) {
            ...
            image.setScaleX(Workspace.mNewScale);
            image.setScaleY(Workspace.mNewScale);
            image.setAlpha(1f);
