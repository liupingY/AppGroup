...
import android.util.Log;//add by zel
import android.view.*;
...
//    private ValueAnimator drop = null;
...
//        setOnHierarchyChangeListener(this);
    ...
    private boolean isEventOverFolder(Folder folder, MotionEvent ev) {
		...
		//add by zhouerlong begin 2015-0728
       /* if (mHitRect.contains((int) ev.getX(), (int) ev.getY())) {
            return true;
        }
        return false;*/
        return folder.isOverLocationCellLayoutItem(ev,mHitRect);
		//add by zhouerlong begin 2015-0728
    }
    
    
    private boolean isEventOverRecommdView(Folder folder, MotionEvent ev) {

        getDescendantRectRelativeToSelf(folder.getmFolderIcon().getRecommdView(), mHitRect);
        
    	View emp = folder.getmFolderIcon().getRecommdView().findViewById(R.id.emptiy);
        if(folder.getmFolderIcon().getRecommdView().getVisibility() != View.VISIBLE) {
        	return false;
        }
		try {
			RecommdLinearLayout r = (RecommdLinearLayout) folder
					.getmFolderIcon().getRecommdView();
			PrizeRecommdView recommd = r.getRecommdScrollView();

			Rect rect = new Rect();
			getDescendantRectRelativeToSelf(recommd, rect);
			boolean contains = rect.contains((int) ev.getX(), (int) ev.getY());
			if (contains && recommd.getChildCount() <= 0) {
				return false;
			}
		} catch (Exception e) {
		}
    ...
    private boolean isEventOverSwitchView(Folder folder, MotionEvent ev) {

        getDescendantRectRelativeToSelf(folder.getmFolderIcon().getSwitchView(), mHitRect);
        if (mHitRect.contains((int) ev.getX(), (int) ev.getY())) {
            return true;
        }
        return false;
    }
    //是否触摸到了foldercheck 范围
    private boolean isEventOverHideView(HideAppView hideView, MotionEvent ev) {
    	
        getDescendantRectRelativeToSelf(hideView, mHitRect);
        return hideView.isOverLocationCellLayoutItem(ev,mHitRect);
    }
    


    //是否触摸到了foldercheck 范围
    private boolean isEventOverFolder(View icon, MotionEvent ev) {
        getDescendantRectRelativeToSelf(icon, mHitRect);
        if (mHitRect.contains((int) ev.getX(), (int) ev.getY())) {
            return true;
        }
        return false;
    ...
    private boolean handleTouchDown(MotionEvent ev, boolean intercept) {
       ...
       /* HideAppView hideView = mLauncher.getHideView();
        VisibleAppsView visibleAppsView = mLauncher.getVisibleView();*/
        if (currentFolder != null && !mLauncher.isFolderClingVisible() && intercept) {
            ...
            if (isEventOverSwitchView(currentFolder, ev)) {
                return false;
            }
            ...
            if (!isEventOverFolder(currentFolder, ev)) {
                ...
                if (isEventOverRecommdView(currentFolder, ev)) {
                    return false;
                }
	...
	/*	if (!isEventOverHideView(hideView, ev)) {
			if (hideView.getParent() != null) {

				getDescendantRectRelativeToSelf(hideView.getMoreView(),
						hitRect);
				if (isEventOverFolder(hideView.getMoreView(), ev)) {
					return false;
				}


				if(visibleAppsView.getAnimationingState()!=hideView.STATE_ANIMATING) {
					if (!isEventOverFolder(visibleAppsView, ev)) {
						if (visibleAppsView.getParent() != null) {
							visibleAppsView.animateClosed();
							return true;
						}
					}
				}
				if(visibleAppsView.getAnimationingState()!=hideView.STATE_ANIMATING&&hideView.getAnimationingState()!=hideView.STATE_ANIMATING) {
					if(!isEventOverFolder(visibleAppsView, ev)&&!isEventOverHideView(hideView, ev)) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
       ...
       /*  if (mLauncher.getworkspace().getState() == Workspace.State.DRAG_MODEL) {
    	   	 if (mLauncher.getworkspace().getSystemUiVisibility() !=View.SYSTEM_UI_FLAG_FULLSCREEN) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if(ev.getPointerCount()>2) {
            ...
            LogUtils.i("zhouerlong", "getPointerCount():::"+ev.getPointerCount());
    ...
    public void animateViewIntoPosition(DragView dragView, final View child,int startDealy,boolean isDecompression,ItemInfo info) {
        animateViewIntoPosition(dragView, child, null,-1,startDealy,isDecompression,info);//add zel
    }

    public void animateViewIntoPosition(DragView dragView, final int[] pos, float alpha,
            float scaleX, float scaleY, int animationEndStyle, Runnable onFinishRunnable,
            int duration,int startDealy,boolean isDecompression,ItemInfo info) {
        Rect r = new Rect();
        ...
        animateViewIntoPosition(dragView, fromX, fromY, pos[0], pos[1], alpha, 1, 1, scaleX, scaleY,
                onFinishRunnable, animationEndStyle, duration, null,startDealy,isDecompression , info);
    }

    public void animateViewIntoPosition(DragView dragView, final View child,
            final Runnable onFinishAnimationRunnable,int duration,int startDealy,boolean isDecompression,ItemInfo info) {
        animateViewIntoPosition(dragView, child, -1, onFinishAnimationRunnable, null,startDealy,isDecompression , info);
    }

    public void animateViewIntoPosition(DragView dragView, final View child, int duration,
            final Runnable onFinishAnimationRunnable, View anchorView,int startDealy,final boolean isDecompression,ItemInfo info) {
       ...
       /* startDealy=200;
        ...
        animateViewIntoPosition(dragView, fromX, fromY, toX, toY, 1, 1, 1, scale, scale,
                onCompleteRunnable, ANIMATION_END_DISAPPEAR, duration, anchorView,startDealy,isDecompression,info);
    }
    ...
    public void animateViewIntoPosition(final DragView view, final int fromX, final int fromY,
            ...
            int animationEndStyle, int duration, View anchorView,int startDealy,boolean isDecompression,ItemInfo info) {
        ...
        animateView(view, from, to, finalAlpha, initScaleX, initScaleY, finalScaleX, finalScaleY, duration,
                null, null, onCompleteRunnable, animationEndStyle, anchorView,startDealy,isDecompression,info);
    }
    ...
    public void animateView(final DragView view, final Rect from, final Rect to,
            ...
            final Runnable onCompleteRunnable, final int animationEndStyle, View anchorView,int startDealy,boolean isDecompression,final ItemInfo info) {
        ...
        final float finalRotationX=view.getRotationX();
        final float finalRotationY = view.getRotationY();
        ...
        AnimatorUpdateListener updateCb = new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                view.setRotationX(finalRotationX*(1-percent));
                view.setRotationY(finalRotationY*(1-percent));
				...
				if (info instanceof PendingAddItemInfo) {
					alpha = /* finalAlpha * alphaPercent + */initAlpha
							* (1 - alphaPercent);
				}
                ...
                int xPos = x - view.getScrollX() + (mAnchorView != null
        ...
        animateView(view, updateCb, duration, interpolator, onCompleteRunnable, animationEndStyle,
                anchorView,startDealy,isDecompression);
    }
    ...
    public void animateView(final DragView view, AnimatorUpdateListener updateCb, int duration,
            ...
            final int animationEndStyle, View anchorView,int startDelay,final boolean isDecompression) {
        // Clean up the previous animations
//        if (drop != null) drop.cancel();
        ...
        final ValueAnimator  drop = new ValueAnimator();
        drop.setInterpolator(interpolator);
        drop.setDuration(duration);
        drop.setFloatValues(0f, 1f);
        drop.addUpdateListener(updateCb);
        ...
        drop.addListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator animation) {
                ...
                case ANIMATION_END_DISAPPEAR:
                    clearAnimatedView(drop,view,isDecompression);
                    break;
                case ANIMATION_END_FADE_OUT:
                    fadeOutDragView(view);
                    break;
    ...
    public void clearAnimatedView(ValueAnimator drop,DragView view,boolean isDecompression) {
        if (drop != null) {
            drop.cancel();
        }
        ...
        if (view != null ) {
        	/*
        	 *  isDecompression 这个变量至关重要  这个变量表示拖拽结束以后所有的收尾工作
        	 * 例如： DragView 的移除
        	 * 	     批量图标总数的移除
        	 * 		批量图标状态结束
        	 */
    ...
    private void fadeOutDragView(final DragView view) {
        mFadeOutAnim = new ValueAnimator();
        mFadeOutAnim.setDuration(this.getResources().getInteger(R.integer.config_dropAnimAndFadeDuration));//?aà?±íê?·?′óμ?ê±??//M by zhouerlong
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mInScrollArea && !LauncherAppState.getInstance().isScreenLarge()) {
            ...
            View left =  workspace.getChildAt(isRtl ? page + 1 : page - 1);
            View right = workspace.getChildAt(isRtl ? page - 1 : page + 1);
            
            CellLayout leftPage = null;
            CellLayout rightPage = null;
			if (right instanceof CellLayout&&left instanceof CellLayout) {
                 rightPage = (CellLayout) right;
                 leftPage =(CellLayout) left;
