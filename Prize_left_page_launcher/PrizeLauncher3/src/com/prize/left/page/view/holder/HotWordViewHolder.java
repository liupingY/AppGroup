...
public class HotWordViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
	...
	private List<HotWordTable> datas = null;
	...
	public HotWordViewHolder(View v) {
		super(v);
		mCtx = v.getContext();
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.img_refresh:
				...
				StatService.trackCustomEvent(mCtx, "CardHot", "");
				break;
			case R.id.txt_title:
			case R.id.txt_title2:
				HotWordTable item = (HotWordTable)v.getTag();
				Intent it;
	...
	public void setModel(HotWordModel m) {
		mModel = m;
	}
	/***
	 * 设置美食(团购)数据
	 * @param ls
	 */
	public void setDatas(List<HotWordTable> ls) {
	...
	private void dealTypeDz() {
		...
		if (min > 0) {
			for (; startIndex < min; startIndex ++) {
				...
				HotWordTable item = datas.get(index);
	 ...
	 * @param data
	 */
	private void initItemView(View v, HotWordTable data) {
		v.setTag(data);
	 ...
	 * @param data
	 */
	private void initItemView(TextView v, HotWordTable data) {
		v.setTag(data);
		v.setOnClickListener(this);
		v.setText(data.word);
	}
	 ...
	 * 获取美食(团队)项数据
	 */
	public List<HotWordTable> getDatas() {
		return datas;
	}
