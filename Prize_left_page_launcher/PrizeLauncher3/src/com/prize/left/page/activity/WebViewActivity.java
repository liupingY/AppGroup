...
import com.prize.left.page.util.CToast;
				...
				try {
					if (url.startsWith("weixin://wap/pay?")) {
						...
						if(isIntent){
						    ...
						    CToast t = CToast.makeText(getApplicationContext(), "未检测到微信客户端，请安装后重试", 6000);
	...
	public void onClick(View v) {
		...
		switch (id) {
			case R.id.btn_refresh:
				if(Utilities.isFastClick(2000)) return;
				mWebView.loadUrl(mUrl);
				break;
			case R.id.btn_back:
				hideInputMethod();
			...
			case R.id.btn_reload:
				...
				mHandler.resetTimes();
				mProgressBar.setProgress(0);
				if(ClientInfo.getInstance(getApplicationContext()).getAPNType(getApplicationContext())!=ClientInfo.NONET){
					mWebView.clearView();
					mWebView.loadUrl(mUrl);
					mWebView.setVisibility(View.VISIBLE);
					mErrorView.setVisibility(View.GONE);
				}else{
					CToast t = CToast.makeText(getApplicationContext(), "网络错误，请检查网络", 1000);
				    t.setGravity(Gravity.BOTTOM, 0, 100);
				    t.show();
				}
				break;
