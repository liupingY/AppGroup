...
/**prize-add-by-zhongweilin-20151119-start**/
import com.mediatek.common.gene.PrizeGene;
...
/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-liuweiquan-20151202-start*/
import android.app.Service;
import android.telephony.PhoneStateListener;
import android.telephony.TelephonyManager;
import android.widget.Toast;
/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-liuweiquan-20151202-end*/
/*prize-public-bug:black screen gesture hide nav-liuweiquan-20160509-start*/
import android.provider.Settings.SettingNotFoundException;
/*prize-public-bug:black screen gesture hide nav-liuweiquan-20160509-end*/
/*prize-public-function:black screen gesture-liuweiquan-20151217-start*/
import android.view.PrizeShowAnimationView;
...
/*prize-public-function:black screen gesture-liuweiquan-20151217-end*/
...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	/*Prize-bug22637-fix go to home after long press shendun fingperint-20161024-start*/
	boolean preKeyguardActive = false;
	/*Prize-bug22637-fix go to home after long press shendun fingperint-20161024-end*/
	...
	/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-liuweiquan-20151202-end*/	
	/*prize-public-function:black screen gesture-liuweiquan-20151217-start*/
	WindowManager gestureWm;
	View rl = null;
	PrizeShowAnimationView mPrizeShowAnimationView;
	String gPackagename =null;
	String gClassname = null;
	/*prize-public-function:black screen gesture-liuweiquan-20151217-end*/
    boolean mHavePendingMediaKeyRepeatWithWakeLock;

    /**prize-add-by-zhongweilin-20150718-start*/
    private PrizeGene mPrizeGene = null;
    /**prize-add-by-zhongweilin-20150718-end*/
  ...
  /*PRIZE-three_slideshot-huangdianjun-2015-04-06-end*/
    ...
    static final boolean DEBUG_FOR_HIDING_NAVBAR = false; //IS_ENG_BUILD;
    ...
    private void printMyLog(String msg) {
        if(MY_DEBUG) {
            Log.d("PhoneWidowManagerPolicy-iroot",  msg);
        }
    }
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        /*prize-lock launcher-modify by wangxianzhen-2016-06-22-start*/
        if(PrizeOption.PRIZE_LOCK_LAUNCHER) {
            mHomeIntent.setClassName("com.android.launcher3", "com.android.launcher3.Launcher");
        }
        /*prize-lock launcher-modify by wangxianzhen-2016-06-22-end*/
        ...
        // Prize-Lucky-Money-helper. add this for populating
        // keyguard. prize-wangyunhe-20161129
        ...
        context.registerReceiver(mFloatWindowReceiver, filter);
		mReadingModeManager = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
		wakeLock = mReadingModeManager.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "PhoneWindowManager.mReadingModeWakeLock");
        ...
        mSystemGestures = new SystemGesturesPointerEventListener(context,
                new SystemGesturesPointerEventListener.Callbacks() {
                    ...
                    public void onSwipeFromTop() {
						...
						if (topApp.contains("com.prize.factorytest")) {
							return;
						}
        ...
        /**prize-add-by-zhongweilin-20150718-start*/
        Log.v(TAG, "****windowManager**TouchResult--->212112121221221 ");
        mPrizeGene = new PrizeGene(mContext,PrizeGene.ANTIFAKE_TOUCH_MODE) {
            public void onAction(int action, Object params) {
                ...
                Log.v(TAG, "****windowManager**TouchResult---> " + mResult.isAntifakeTouch);
                if(!mResult.isAntifakeTouch){
                    prizeSleepGesture();
                    Log.v(TAG, "****windowManager** AntifakeTouch---> Gene.stop()");
                }
                mPrizeGene.stop();
            }
        };
	...
	/**prize-add-game-reading-mode-liuweiquan-20160415-start*/
	private PowerManager mReadingModeManager = null;
    ...
    public int selectAnimationLw(WindowState win, int transit) {
        ...
        if (win == mStatusBar) {
            ...
            if (mNavigationBarOnBottom) {
                if (transit == TRANSIT_EXIT
                        || transit == TRANSIT_HIDE) {
                    //add for hiding nav bar. prize-linkh-20150806
                    if(mSupportHidingNavBar) {
                        return -1;
                    } //end...  
...
//add for Double taping to close screen off. prize-wangyunhe
   ...
   private void startFlash() { 
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        /// M: Add log.
        if ((false == IS_USER_BUILD) || DEBUG_INPUT || true) {
	...
	//end....
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
			/**prize-add_factorytest-by-liup-20150415-start*/
				String topApp = getTopApp();
				if (topApp.equals("com.prize.factorytest.Key.Key")) {
					return 0;
				}
			...
			/*PRIZE-liyu-fingerprint enrolling activyty-20160813-start*/
				if(topApp.equals("com.android.settings.fingerprint.PrizeFingerprintEnrollEnrolling")){
					return 0;
				}
			/*PRIZE-liyu-fingerprint enrolling activyty-20160813-end*/
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
        ...
        if (isDefaultDisplay) {
            ...
            //add for hiding nav bar. prize-linkh-20150715    
            //we always set this value. 
            mNavigationBarOnBottom = (!mNavigationBarCanMove || displayWidth < displayHeight);
            
            //add for hiding nav bar. prize-linkh-20150720
            boolean shouldCalNavBar = mNavigationBar != null;
            // If it's a mBack device, then we need this calculation for force-hiding nav bar.
            if((mSupportHidingNavBar || SUPPORT_NAV_BAR_FOR_MBACK_DEVICE) && shouldCalNavBar) {
                ...
                if (DEBUG) {
                    Slog.d(TAG, "isNavBarGone=" + isNavBarGone);
                    if (attrs.navBarGoneReason != null) {
                        Slog.d(TAG, "navBarGoneReason=" + attrs.navBarGoneReason);
                    }
                }
                if(isNavBarGone) {                    
                    ...
                    if (DEBUG) {
                        Slog.d(TAG, "shouldCalNavBar=" + shouldCalNavBar);
                    }
                }
            }//END....
            
            if (shouldCalNavBar) {//(mNavigationBar != null) { //mod for hiding nav bar. prize-linkh-20150715    
	...
	private void prizeSleepGesture(){
		...
		Log.e("lwq","result1 = " + result.substring(0,4));
		...
		if (cursor != null && cursor.moveToFirst()) {
			...
			/*prize-public-function:black screen gesture-liuweiquan-20151217-start*/
			if(null!=gClassname&&rl!=null)
				return;
			gPackagename = cursor.getString(cursor.getColumnIndex("packagename"));
			gClassname = cursor.getString(cursor.getColumnIndex("classname"));
			Log.e("lwq","packagename =" + gPackagename + " classname = " + gClassname);
			/*prize-public-function:black screen gesture-liuweiquan-20151217-end*/
			Log.e("liup", "item=" + item + " packagename =" + packagename + " onoff= " + onoff);
			if(onoff == 1){
				/*PRIZE-prize-public-bug:bugid:12166 black screen gesture-liuweiquan-20160310-start*/
				if(getPhoneCallState()==0)
					isPhoneIdle=true;
				else
					isPhoneIdle=false;
				/*PRIZE-prize-public-bug:bugid:12166 black screen gesture-liuweiquan-20160310-end*/
				/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-modifya:if(packagename.equals("mPreviousMusic"))-liuweiquan-20151217*/
				if(packagename.equals("mPreviousMusic")&&isPhoneIdle){
					Intent service = new Intent();
					...
					mContext.sendBroadcast(intent);
				/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-modifya:if(packagename.equals("mPlayPauseMusic"))-liuweiquan-20151217*/
				}else if(packagename.equals("mPlayPauseMusic")&&isPhoneIdle){
					Intent service = new Intent();
					//service.setClassName("com.android.music", "com.android.music.MediaPlaybackService");
					service.setClassName("com.prize.music", "com.prize.music.service.ApolloService");
					mContext.startService(service);
					...
					mContext.sendBroadcast(intent);
				/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-modifya:if(packagename.equals("mNextMusic"))-liuweiquan-20151217*/
				}else if(packagename.equals("mNextMusic")&&isPhoneIdle){
					Intent service = new Intent();
					//service.setClassName("com.android.music", "com.android.music.MediaPlaybackService");
					service.setClassName("com.prize.music", "com.prize.music.service.ApolloService");
					mContext.startService(service);
					...
					/*PRIZE-prize-public-bug:bugid:9149 Call black screen gesture-modifya:if(packagename.equals("awake") || mKeyguardSecure)-liuweiquan-20151202*/
					if(packagename.equals("awake") || mKeyguardSecure||!isPhoneIdle){
					...
					/*prize-public-function:black screen gesture-liuweiquan-20151217-start*/					
					//startGestureAnimation(result.substring(0,4));
	...
	/*prize-public-function:black screen gesture-liuweiquan-20151217-start*/
	private void startGestureAnimation(String code){	

		rl=LayoutInflater.from(mContext).inflate(com.prize.internal.R.layout.activity_prize_animation, null);
		mPrizeShowAnimationView =(PrizeShowAnimationView) rl.findViewById(com.prize.internal.R.id.empty);
		
		ViewTreeObserver textViewTreeObserver=rl.getViewTreeObserver();
		textViewTreeObserver.addOnGlobalDrawDoneListener(new OnGlobalDrawDoneListener() {
            public void onGlobalDrawDone() {
                //Do your operations here. 
                Log.e("lwq","--ViewTreeObserver onGlobalDrawDone---");    
				rl.getViewTreeObserver().removeOnGlobalDrawDoneListener(this);		
				mSleepGestureWakeLock.acquire();
				mSleepGestureWakeLock.release();				
				if(mPrizeShowAnimationView != null){
					selectGestureAnim(result.substring(0,4));
				...
				new Thread(){
					public void run(){
						...
						selectGestureApp(result.substring(0,4));
					}
				}.start(); 				
            }
        });
		
		gestureWm = (WindowManager) mContext.getSystemService(Context.WINDOW_SERVICE);
		...
		Log.e("lwq","PhoneWindowManager.java showAnimation gestureWm.addView");
        gestureWm.addView(rl, params);						
	}
	private void selectGestureAnim(String code){
		TypedArray typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_c);
		Log.e("lwq","selectAnimGesture string = " + code);
		if(code.equals("0xcc")){//double
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_double);	
		}
		else if(code.equals("0xaa")){//right
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_right);
		}
		else if(code.equals("0xba")){//up
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_up);
		}
		else if(code.equals("0xab")){//down
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_down);
		}
		else if(code.equals("0xbb")){//left
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_left);
		}
		else if(code.equals("0x6f")){//O
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_circle);
		}
		else if(code.equals("0x76")){//v
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_v);
		}
		else if(code.equals("0x77")){//w
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_w);
		}
		else if(code.equals("0x7a")){//z
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_z);
		}
		else if(code.equals("0x73")){//s
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_s);
		}
		else if(code.equals("0x6d")){//m
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_m);
		}
		else if(code.equals("0x65")){//e
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_e);
		}
		else if(code.equals("0x63")){//c
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_c);
		}
		else if(code.equals("0x5e")){//^
			typedArray = mContext.getResources().obtainTypedArray(com.prize.internal.R.array.prize_sleep_arror_up);
		}
		for( int index = 0; index < typedArray.length(); index++ ){
			mPrizeShowAnimationView.addImageId(typedArray.getResourceId( index, 0 ));
		}
	}
	private void selectGestureApp(String code){
		String packagename = gPackagename;
		...
		if(classname.equals("com.android.camera.CameraLauncher")){
			...
			gClassname = "com.android.camera.CameraActivity";
		}	
		if(getTopPackageName().equals(gPackagename)||gPackagename.equals("unlock")){
			if(null!=rl){
				gestureWm.removeView(rl);
				rl=null;
				gPackagename=null;
				gClassname=null;
			}	
		}else{
			Log.e("lwq","startActivity:"+classname+" elapsedRealtime:"+SystemClock.elapsedRealtime());
			isGestureNeed=true;			
			try {
				Intent intent = new Intent();
				intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
	...
	public static boolean isGestureNeed=false;
            ...
            mTimerTask = new TimerTask() {  
                ...
                public void run() {
					if((SystemClock.elapsedRealtime()-elapsedRealtime)>3000){
						...
						Log.e("lwq","PhoneWindowManager Remove gesture window timeout elapsedRealtime ="+SystemClock.elapsedRealtime());
					}
					Log.e("lwq","PhoneWindowManager TimerTask run topactivity = "+getRunningActivityName(mContext)+" toppackage ="+getTopPackageName()+" elapsedRealtime ="+SystemClock.elapsedRealtime());			
					if(!isGestureNeed&&null!=gPackagename&&rl!=null&&getTopPackageName().equals(gPackagename)){
						stopTimer();
						Log.e("lwq","PhoneWindowManager postDelayed elapsedRealtime ="+SystemClock.elapsedRealtime());							
						mHandler.postDelayed(new Runnable() {
							...
							public void run() {									
								...
								rl=null;
								elapsedRealtime=0;
								Log.e("lwq","PhoneWindowManager finishDrawingWindow removeGestureWindow elapsedRealtime ="+SystemClock.elapsedRealtime());
							}
						},150);		
					}					  					             							
                }  
	...
	BroadcastReceiver phoneReceiver =new BroadcastReceiver(){
		...
		public void onReceive(Context context, Intent intent) {		
			if(intent.getAction().equals(Intent.ACTION_NEW_OUTGOING_CALL)){
				...
				Log.d("lwq", "call OUT:" + phoneNumber);  
			}else{			
		...
		PhoneStateListener listener=new PhoneStateListener(){				 
			...
			public void onCallStateChanged(int state, String incomingNumber) {
				...
				switch(state){
					...
					isPhoneIdle=true;
					Log.d("lwq", "CALL_STATE_IDLE");  
					break;
					case TelephonyManager.CALL_STATE_OFFHOOK:
					isPhoneIdle=false;
					Log.d("lwq", "CALL_STATE_OFFHOOK"); 
					break;
					case TelephonyManager.CALL_STATE_RINGING:
					isPhoneIdle=false;
					Log.d("lwq", "CALL_STATE_RINGING:"+incomingNumber); 
					break;
				}
			}
		};		 
	...
	private int getPhoneCallState() {
		int phoneCallState = 0;
		TelephonyManager tm = (TelephonyManager)mContext.getSystemService(Service.TELEPHONY_SERVICE);  	
		phoneCallState=tm.getCallState();
		Log.e("lwq","PhoneWindowManager getCallState = "+phoneCallState);
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
		...
		if(PrizeOption.PRIZE_SLEEP_GESTURE){
			if(keyCode == GESTURE_KEY_CODE && down){
                /**prize-modify-by-zhongweilin-20150718-start*/
                Log.v(TAG, "****windowManager*** isAntiFakeTouchOn ********");
                if (PrizeOption.PRIZE_ANTIFAKE_TOUCH) {
                    int antifakeTouch = Settings.System.getInt(mContext.getContentResolver(),
                            Settings.System.PRIZE_ANTIFAKE_TOUCH, 0);
                    Log.v(TAG, "****window***AntiFakeTouchOn ~~~ antifakeTouch == " + antifakeTouch);
                    if (antifakeTouch == 1) {
                ...
                /**prize-modify-by-zhongweilin-20150718-end*/
			}
		}
        ...
        /// M: Add log.
	//add haptic feedback for home&menu&back key. prize-linkh-20150519
	useHapticFeedback = down && 
			(event.getRepeatCount() == 0) &&
			(keyCode == KeyEvent.KEYCODE_HOME || 
	...
	//end..............
	    ...
	    //add start:lihuangyuan,for HOME key wakeup system
	    case KeyEvent.KEYCODE_HOME:{
			//result &= ~ACTION_PASS_TO_USER;
                	if(!isScreenOn())isWakeKey = true; // modify by lihuangyuan,for only wake-up at screen off-2017-03-31
					/*Prize-bug22637-fix go to home after long press shendun fingperint-20161024-start*/
					if(preKeyguardActive && !keyguardActive && !down) {
						preKeyguardActive = keyguardActive;
						return 0;
					}
					preKeyguardActive = keyguardActive;
					/*Prize-bug22637-fix go to home after long press shendun fingperint-20161024-end*/
        ...
        if (useHapticFeedback) {
			final boolean bDblclickSleep = Settings.System.getInt(mContext.getContentResolver(),Settings.System.PRIZE_DBLCLICK_SLEEP,0) == 1;
			 if(bDblclickSleep && keyCode == KeyEvent.KEYCODE_HOME && sendKeyEventTime == event.getEventTime()) {
				printMyLog("It's our injected home key event. Ignore this haptic feedback. ");
			} else { //end....
				 performHapticFeedbackLw(null, HapticFeedbackConstants.VIRTUAL_KEY, false);
			}
        }

	//play click effect for home&menu&back key. prize-linkh-20150519
	boolean playFeedback = !down &&
			(keyCode == KeyEvent.KEYCODE_HOME || 
				keyCode == KeyEvent.KEYCODE_MENU ||
				keyCode == KeyEvent.KEYCODE_BACK);
	if(playFeedback) {
    ...
    private boolean isWakeKeyWhenScreenOff(int keyCode) {
        switch (keyCode) {
			...
			case KeyEvent.KEYCODE_BACK: //add by liyu
                return false;
        }
        return true;
    }
    ...
    //add for hiding nav bar. Monitor motion event before passing to user. prize-linkh-20150805
    ...
    private int updateSystemBarsLw(WindowState win, int oldVis, int vis) {
        ...
        if (PrizeOption.PRIZE_STATUSBAR_INVERSE_COLOR && (vis & View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR) != 0
            ...
            int mask = View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR | View.SYSTEM_UI_TRANSPARENT;
            if (DEBUG) {
