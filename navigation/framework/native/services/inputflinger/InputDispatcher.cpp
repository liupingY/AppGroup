...
//add for monitoring motion event before passing to user. prize-linkh-20150805      
static bool mEnableMonitorMotionEvent= false;
static bool isSupportHidingNavBar() {
    char value[PROPERTY_VALUE_MAX];
    int length = property_get("ro.support_hiding_navbar", value, NULL);
    if (length > 0) {
        if (!strcmp("1", value)) {
            return true;
        }
...
InputDispatcher::InputDispatcher(const sp<InputDispatcherPolicyInterface>& policy) :
    ...
    //add for monitoring motion event before passing to user. prize-linkh-20150806     
    mEnableMonitorMotionEvent = isSupportHidingNavBar();
    if(mEnableMonitorMotionEvent) {
        ALOGD("mEnableMonitorMotionEvent = true");  
    }
}
...
void InputDispatcher::notifyMotion(const NotifyMotionArgs* args) {
    ...
    if(mEnableMonitorMotionEvent) {
        ...
        motionEvent.initialize(args->deviceId, args->source, args->action, args->actionButton,
                    args->flags, args->edgeFlags, args->metaState, args->buttonState,
                    0, 0, args->xPrecision, args->yPrecision,
                    args->downTime, args->eventTime,
                    args->pointerCount, args->pointerProperties, args->pointerCoords);
        mPolicy->interceptMotionBeforeQueueing(&motionEvent, args->eventTime, /*byref*/policyFlags);
    } else { //end....
...
int32_t InputDispatcher::injectInputEvent(const InputEvent* event, int32_t displayId,
    ...
    case AINPUT_EVENT_TYPE_MOTION: {
        ...
        if (!(policyFlags & POLICY_FLAG_FILTERED)) {
            ...
            //add for monitoring motion event before passing to user. prize-linkh-20150805
            if(mEnableMonitorMotionEvent) {
                mPolicy->interceptMotionBeforeQueueing(motionEvent, eventTime, /*byref*/ policyFlags);
            } else {
